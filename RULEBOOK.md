...
**9:50 — flat, no trade, first direct RVOL clear of the live system.** QQQ and SPY joined the breakout list (both below their opening-range lows: QQQ $718.16<$718.45, SPY $760.35<$760.59, both bear); XBI still breaking below ($157.025 vs $157.160). **TLT cleared 1.5× RVOL directly this checkpoint — ~1.638× at the 20-minute mark** — the first time step 1 has flagged a proxy on the real threshold rather than needing the v3.79 ratio fallback. Since at least one proxy cleared 1.5×, no fallback ranking runs at all this checkpoint — only TLT is checked at step 2. **TLT is inside its own $81.2745-$81.470 range** (live $81.315) — fails step 2. No other proxy is checked under this branch of the rule, even though QQQ, SPY, and XBI are all real breakouts right now — step 1's direct-clear path doesn't fall through to the fallback ranking when something already cleared 1.5×, by design (the fallback exists only for "nothing cleared the bar"). Correct no-trade, but flagging the mechanic plainly since it's the first live case of it: a real, isolated volume spike (TLT here) can eat the checkpoint's single step-2 check even while it isn't the proxy actually moving. (Other RVOL readings this checkpoint: QQQ ~1.10×, XBI ~1.02×, IWM ~1.02×, SPY ~0.96×, XRT ~0.76×, SOXX ~0.88×, XLF ~0.67×, XLE ~0.62×, GLD ~0.38×.)

**9:55 — flat, no trade, same mechanic repeats, broader breakout field.** The bearish move has broadened: QQQ ($718.11<$718.45), SPY ($760.06<$760.59, extending), IWM (newly below $283.495 at $283.25), GLD (newly, marginally below $398.985 at $398.9098), and XBI ($156.87<$157.160, extending) are now all breaking bearish — five of ten proxies. **TLT cleared 1.5× RVOL directly again, more strongly this time — ~2.070× at the 25-minute mark** (one 5-min bar, 13:45-13:50 UTC, printed 1.618M shares alone — plausibly the 9:45am ET Services PMI release moving Treasuries specifically, per this morning's economic calendar). Same mechanic as 9:50: since TLT cleared 1.5× directly, no fallback ranking runs, and **TLT itself is still inside its own $81.2745-$81.470 range** (live $81.32) — fails step 2. None of the five real breakouts get checked. Correct no-trade, but two checkpoints running now where an isolated TLT volume spike (plausibly macro-data-driven, not a TLT-specific move) has blocked the checkpoint from ever looking at a broadening, genuine equity-side selloff. Worth watching — not changing anything over two data points, per D4's discipline against overfitting to a short pattern, but flagging clearly for the governor's awareness since the mechanic is now repeating.

**10:00 — flat, no trade, third consecutive checkpoint with the same mechanic — escalating for the record.** The bearish breakout field held: QQQ, SPY, IWM, GLD, and XBI all still breaking below their opening-range lows, unchanged from 9:55. **TLT cleared 1.5× RVOL directly a third straight time — ~1.78× at the 30-minute mark.** TLT itself remains inside its own range, but **the gap has narrowed sharply: live $81.2896 vs its own $81.2745 low, only $0.0151 away** — the closest it's been all morning, consistent with the broader bearish pressure finally reaching Treasuries too. Correct no-trade again, same reason (direct-clear path doesn't fall through to the fallback, so the five real breakouts go unchecked a third time). **What to watch at 10:05: if TLT itself breaks below $81.2745 while still clearing 1.5× RVOL, it clears both C1 steps outright — bearish, candidate TMV** — a real, mechanically valid entry, not a rule problem. This is now a three-checkpoint pattern (9:50, 9:55, 10:00) worth the governor's attention regardless of how it resolves: an isolated-instrument volume spike has structurally blocked five genuine breakouts from ever being checked, which is a real gap in how the direct-clear branch interacts with a broad, multi-proxy move — distinct from the fallback-ranking mismatches seen Thursday.

**10:05 — missed, no capital at risk.** This checkpoint's trigger never delivered a notification into the session (found still enabled and past-due at the 10:10 read, per A2's "past-due enabled trigger = missed checkpoint" rule) — deleted after confirming, at 10:10, that the account had stayed flat with zero orders since 10:00, so nothing was left unmanaged during the gap. Doing the 10:10 read below in its place; no separate retroactive read attempted since the data would only be seconds different from 10:10's own.

**10:10 — flat, no trade, fourth checkpoint with the same TLT-RVOL mechanic.** GLD recovered back inside its range; QQQ, SPY, IWM, and XBI are still breaking bearish (four of ten now). **TLT cleared 1.5× RVOL directly again — ~1.61× at the 40-minute mark** — but this time moved away from its own low rather than toward it (live $81.325 vs its $81.2745 low, a $0.0505 gap, wider than 10:00's $0.0151). Still inside its range, still fails step 2, same mechanic. Correct no-trade. The immediate "TLT about to break" tension from 10:00 has eased for now, but the underlying gap (an isolated volume spike blocking a real multi-proxy move from ever being checked) is unchanged and still worth the governor's attention regardless of how today resolves.

**10:15 — flat, no trade, fifth checkpoint, same result via a different mechanism.** The bearish field widened again: QQQ, SPY, IWM, and XBI still breaking, plus **XRT newly, marginally breaking below its low** ($82.31 vs $82.320) — five of ten now. **TLT's RVOL dropped just under the threshold this checkpoint — ~1.486× at the 45-minute mark** — the first time since 9:50 it hasn't cleared 1.5× directly. Since nothing cleared the bar, the v3.79 ratio fallback runs instead of the direct-clear path, but **TLT still tops that ranking** (1.486× vs IWM's 0.904× next-best) and is carried into step 2 alone — same outcome as the last four checkpoints, just via the fallback branch instead of the direct one. TLT remains inside its own range (live $81.295, gap back down to $0.0205) — fails step 2. Correct no-trade, fifth checkpoint running where TLT (spiking on volume, not on its own price) is the only name checked while a genuinely broadening five-proxy bearish move goes unchecked either way, whichever branch of step 1 is active.

**10:20 — ENTRY. First trade of the day: TMV, bearish leg off TLT.** TLT itself finally joined the breakout it had been eating the checkpoint for — **live $81.215/$81.205, below its own $81.2745 low.** XLF flipped bullish (newly above $55.765); QQQ, SPY, IWM, XBI still breaking bearish; GLD newly breaking bearish too. RVOL at the 50-minute mark: TLT ~1.39× (below 1.5× this time, ranked: TLT 1.39×, IWM ~0.90×, QQQ ~0.82×, SPY ~0.76×, XBI ~0.74×, SOXX/XLF/XRT ~0.59-0.61×, XLE ~0.50×, GLD ~0.32×). **v3.79 fallback**: TLT tops the ranking — and this time also clears step 2, bearish, candidate TMV. Only one row cleared both steps. Re-verified TLT immediately (live $81.235, still below $81.2745) — held. Profiled TMV on 25 daily sessions: stop 2.50%, target 3.75% (clamped, mfe_to_target 5.09 — target likely unreachable per the profile's own flag), stall 0.11%. Settled cash $502.34. `review_equity_order` clean, no alerts. Placed marketable limit buy, 11sh (floor(502.34/44.17)) @ $44.22 limit — **filled 11sh @ $44.1391**, $0.0809/share better than the limit. Total cost $485.53. Placed protective stop_market at $43.04 (2.50% below fill) — confirmed resting. Next checkpoint (10:25) is ≤5min out, no ad hoc trigger needed. **Today's trade count: 1 of 3.** Intended exit: stop $43.04, ratchet per B2 as TMV runs, target ~$45.82 (+3.75%, likely unreachable per profile), same-day close by 11:00 regardless. Falsifiable pre-commit for 10:25: watch for TLT reversing back above its own $81.2745 opening-range low — a genuine bounce, not noise at the line, same discipline as Thursday's TZA trade.

**10:25 — MANAGE, position held, first ratchet.** TLT pre-commit not triggered — live $81.20, still well below its $81.2745 low, no reversal. TMV live $44.18, above entry fill $44.1391 (in profit) — **profit gate open, ratchet fires.** `run_high` = $44.180 (1-min bars since entry, 14:23-14:25 UTC). `candidate_stop` = $44.180 × (1 − 2×0.11%) = $44.0828 → $44.08. Live staleness check: live $44.18 > candidate — not stale, proceed. Cancelled the resting $43.04 stop (confirmed cancelled, no fill raced it), placed and confirmed a new stop at **$44.08** (well above the move-minimum 0.22%). Stop raised $43.04 → $44.08 on the first ratchet. Falsifiable pre-commit for 10:30: same as before — TLT reversing back above $81.2745 is the reversal signal to watch for ahead of the mechanical stop.

**10:30 — MANAGE, position held, second ratchet.** TLT still no reversal — live $81.165, further below its $81.2745 low, the move extending. TMV live $44.225, still in profit — ratchet fires again. `run_high` = $44.2314 (1-min bars, 14:25-14:30 UTC). `candidate_stop` = $44.2314 × (1 − 2×0.11%) = $44.1341 → $44.13. Live staleness check: live $44.225 > candidate — not stale. Cancelled the resting $44.08 stop (confirmed cancelled, no fill raced it), placed and confirmed a new stop at **$44.13**. Stop raised $44.08 → $44.13. Position now +0.20% unrealized above the fill, trailing tighter as TMV climbs with TLT's decline. Falsifiable pre-commit for 10:35: unchanged — TLT back above $81.2745.

**10:35 — MANAGE, position held, stop unchanged.** TLT no reversal — live $81.175, still well below $81.2745. TMV live $44.215, still in profit vs fill $44.1391, but `run_high` unchanged at $44.2314 (no new high in the 14:30-14:35 bars — TMV pulled back and chopped sideways this interval). `candidate_stop` recomputes to the same $44.1341 → $44.13, equal to the current stop — `new_stop = max(44.13, 44.13) = 44.13`, no cancel/replace needed, consistent with B2's "never tighten on a flat print." Stop stays at $44.13. Falsifiable pre-commit for 10:40: unchanged — TLT back above $81.2745.

**10:40 — EXIT discovered, stop triggered cleanly on its own.** Position check came back flat with no resting orders — the $44.13 stop had filled at 14:39:12 UTC, 11sh @ $44.1221, four minutes before this checkpoint. TMV had run up to a true high of $44.250 (10:36) before reversing sharply through the resting stop in its final minute, filling cleanly with no manual intervention needed — the same "stop discovered doing its job" pattern as SOXL on 9/16. True MAE/MFE over the ~16-minute hold: high $44.250 (MFE +0.25%), low $44.125 at the exit itself (MAE +0.03%) — the position spent nearly the whole hold in profit, giving it back only at the very end. Net: **-$0.19, -0.039%**, r=-0.015 — effectively a breakeven scratch, well inside E1's -1.0% threshold (loss streak stays 0 of 3). Daily trade count stays 1 of 3 (exits don't add to the count). Logged to `archive/trades.csv`. Account now flat, $502.15. Worth restating alongside the standing E6 note: TLT's own breakout happened to be real and tradeable this time, but the parallel QQQ/SPY/IWM/XBI breakouts from the same TLT-blocked stretch (9:50-10:20) were never checked at all — this trade landing near-breakeven doesn't resolve whether that gap cost or saved anything on this particular morning.

**10:45 — ENTRY. Re-entered TMV, same signal, second trade of the day.** QQQ, SPY, and IWM still breaking bearish; XLE newly broke bullish; XLF/GLD/XBI/XRT all recovered back inside their ranges. RVOL at the 75-minute mark, all under 1.5×: TLT ~1.14× (top), IWM ~0.75×, QQQ ~0.65×, SPY ~0.63×, XBI ~0.56×, XLF ~0.50×, SOXX ~0.47×, XRT ~0.44×, XLE ~0.44×, GLD ~0.27×. **v3.79 fallback: TLT tops the ranking again and is still below its own $81.2745 low** (live $81.255) — clears both steps, bearish, candidate TMV. **Checked C12 before proceeding**: the 10:40 exit's `pnl_pct_position` was -0.039%, inside E1's scratch band (better than -1.0%) — not a "losing exit" under v3.68's exclusion, which the RKLB precedent (a real -2.01% loss) shows was written for actual losses, not scratches. TMV is therefore not excluded, and step 0's profitable-exit shortcut doesn't apply either since the prior exit wasn't profitable — this runs as an ordinary fresh C1 read, which is what the regular 10:45 slot already does. Re-verified TLT twice (review-time and immediately pre-placement, both still below $81.2745) — held both times. Reused TMV's profile from the 10:20 entry (unchanged intraday: stop 2.50%, target 3.75%, stall 0.11%). Settled cash $502.15. `review_equity_order` clean. Placed marketable limit buy, 11sh (floor(502.15/44.11)) @ $44.16 limit — **filled 11sh @ $44.1099**, favorable. Total cost $485.21. Placed protective stop_market at $43.01 (2.50% below fill) — confirmed resting. **Today's trade count: 2 of 3** — the 3rd would be discretionary per A1. Intended exit: stop $43.01, ratchet per B2, target ~$45.76 (+3.75%, likely unreachable per profile), same-day close by 11:00. Falsifiable pre-commit for 10:50: TLT reversing back above $81.2745, same discipline as the last two trades.

**10:50 — MANAGE, position held, marginal TLT read treated as noise, not a confirmed reversal.** TLT printed $81.279 (checked twice, both reads identical) — technically above the $81.2745 line, but by only $0.0045, with the bid ($81.27) still sitting on the original (below-the-line) side; the spread straddles the boundary rather than showing a clean move through it. This is the same "noise at the line" shape flagged explicitly when the pre-commit was written (and the shape of Thursday's 10:05 round-trip), not the decisive, full-cent-plus move that actually triggered the real exits on this signal before (TZA's $0.51 IWM reversal, this same TLT/TMV pair's earlier entries). Treating it as noise, not honoring an exit on it. TMV live $44.07, below entry fill $44.1099 — **profit gate closed, ratchet does not fire**, stop stays unconditionally at $43.01 per v3.63 regardless of the TLT read. Holding. Falsifiable pre-commit for 10:55, restated more precisely: a **clean** TLT print above $81.2745 with the bid also above the line (not just the last trade straddling it) — the same bar the entry and the TZA exit were held to.

**10:55 — MANAGE, position held, no reversal, last entry-eligible checkpoint of the day.** TLT retreated back below its low ($81.265 vs $81.2745), confirming the 10:50 print was noise, not a genuine reversal. TMV live $44.06, still below entry fill $44.1099 — profit gate stays closed, no ratchet, stop unchanged at $43.01. No B3 exit condition fired. This is the last entry-eligible slot (C9/v3.77) but irrelevant this checkpoint since a position is already open (E2's one-position rule) — no new candidate considered. Holding into 11:00. Falsifiable pre-commit for the 11:00 close: same clean-print standard as before, though 11:00 closes the position outright regardless if still open at that point (B2/v3.58).

**11:00 — CLOSE. Final ratchet, then direct market sell (double duty, v3.77).** TMV had turned decisively higher: live $44.32, well above entry fill $44.1099 (in solid profit). `run_high` = $44.320 (1-min bars, 14:55-15:00 UTC). `candidate_stop` = $44.320 × (1 − 2×0.11%) = $44.2225 → $44.22. Cancelled the resting $43.01 stop (confirmed cancelled), recorded the ratcheted level for the log, then proceeded straight to the close mechanic rather than resting the new stop — no ratchet-then-gap, per B2's 11:00 rule. Direct market sell, 11sh — **filled @ $44.265**, above the ratcheted level, on a real move rather than a stop trigger. Net: **+$1.71, +0.352%**, r=0.141 — a real winner. True MAE/MFE over the ~14-minute hold: true low $43.955 (14:52 UTC, MAE +0.35% — the position spent much of the hold underwater before turning), true high $44.320 (15:00, at the close itself, MFE +0.48%). Logged to `archive/trades.csv`. Account now flat, confirmed via `get_equity_positions`.

**Full D3 close report for Friday 2026-09-18.** Total value **$503.86** (up from $502.34 at 9:00 — net +$1.52 on the day). All-time realized P&L $3.90 (`get_realized_pnl`). **Two trades today, both on the same TLT/TMV signal**: TZA-style first round (10:23-10:39) closed as a near-breakeven scratch (-$0.19, -0.039%), second round (10:48-11:01) closed as a real winner (+$1.71, +0.352%) — net for the day +$1.52 from trading, matching the account delta exactly. **Loss streak: 0 of 3** (unaffected all day — no trade cleared the -1.0% real-loss threshold). **Daily trade count: finished at 2 of 3** (2 entries, well under the cap, no 3rd discretionary attempt was ever offered a qualifying setup). No resting orders, flat overnight. Standing E6 items from today: the MCP connectivity gap (Thursday, resolved) and the TLT-RVOL-blocks-a-broader-move pattern (5+ checkpoints this morning) both remain on record for the governor's awareness — today's trading resolved cleanly regardless.

**Primary arming: Monday 2026-09-21's full grid, skipping the weekend (D1's "Friday arms Monday").** Confirmed today (2026-09-18) is a Friday and tomorrow is Saturday — arming Monday next, using D1's current 19-slot grid as the source of truth. All 19 slots created (9:00 through 8:00pm) and verified complete via a fresh `list_triggers` read — no gaps, no duplicates. Also confirmed no stray enabled triggers remain from today except the still-pending 8:00pm backup (correct — it hasn't fired yet).
