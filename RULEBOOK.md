...
**Current mitigation is manual, not systemic, and is no longer proving reliably fast enough for either failure mode.** After every stop placement, verify it actually landed via `get_equity_orders` before considering the position protected; retry immediately if it didn't, and if the retry keeps failing, re-check the live price before retrying blind — a rejection can mean the stop level itself is stale, not just that the placement call needs repeating. This has caught every incident so far (2026-08-24 ×2, 2026-08-25 ×2, one per failure mode), but each incident has needed a human-speed sequence of read-decide-act inside a live, moving market, and 2026-08-25 alone produced one real-exposure gap (SMCX) and one unprotected ~30-second gap that got lucky (UUUU).

**Worth a harder look, no longer just "worth" it — this needs building before the next occurrence, not after.** An automatic retry-and-verify wrapper around stop placement, covering both known failure modes: place, confirm via a follow-up read, and on anything other than a confirmed live order — re-check the current price first (in case the stop level itself needs recomputing against a moved market, not just resubmitting), then retry, without waiting for a human or a lucky manual check to notice. Four incidents across two failure modes in two days is the evidence that manual catch-and-retry was never a solved problem, just an untested one that has now been tested twice.
