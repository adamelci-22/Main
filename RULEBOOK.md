...
**Wednesday 2026-09-16, pre-market (still before 9:00) — further correction, v3.76.** Governor clarified that the RVOL/ORB test v3.72 just moved onto "all twenty tickers directly" was wrong: it belongs on the ten proxies (the "core ETF holding," never traded), same as the original v3.71 design intended, but now as the full three-step test rather than just a day-change gate. The proxy's own Opening Range Breakout direction — above its 15-min high (bullish) or below its 15-min low (bearish) — is what picks the leveraged long vs. inverse leg to actually enter; ATR-expansion rank is computed on the proxy's bars too. B6's range tracking and C8's pre-placement re-check both moved with it. Still today's 9:00 research checkpoint hasn't run yet, so this lands before the system's first live use, not mid-day. Full detail in RULEBOOK.md commit history (v3.76).

**Wednesday 2026-09-16, pre-market (still before 9:00) — close time change, v3.77.** Governor moved the close 15 minutes earlier, from 11:15 back to 11:00 (double duty, same mechanic as v3.73 — last ratchet, then direct market sell if still open, report, arm tomorrow). Entry window shortens to 9:45-10:55; the 11:05 and 11:10 slots are removed from the grid outright. Today's already-armed chain updated live to match: deleted the 11:05 (`trig_01KhfsejM1P3eJuK2suxbLFC`) and 11:10 (`trig_01DLM2XqsurPyuN8Cr5pctuo`) triggers, deleted the old 11:15 close+arming trigger (`trig_01J8PNjtkYV7HAS2kFpzuzZk`), and updated the existing 11:00 trigger (`trig_0176S6aEDHN8cE4gVuJ8ixJw`) to carry the close+primary-arming duty instead of a plain management read. Still before 9:00, so this lands before the system's first live use today. Full detail in RULEBOOK.md commit history (v3.77).

**Wednesday 2026-09-16, 9:00 research — first live session under v3.71-v3.78 (Core Ten, proxy-gated RVOL-ORB-ATR, 11:00 close, daily trade cap).** Full detail on Tuesday 9/15's two RKLB round-trips (both losses) archived in `archive/trades.csv` and git history — not restated here per D3's "authoritative log, not a third copy" discipline.

- **A1 checks: loss streak 2 of 3** (RKLB#1 -2.01% and RKLB#2 -1.59%, both 9/15 — one more sub-(-1.0%) closed trade trips the breaker, no new-entry impact yet). **Position: flat, no resting orders** (`get_equity_positions` empty, `get_equity_orders` shows zero non-terminal orders). Total value/cash/buying power **$455.66**, no pending deposits, no unsettled funds (`get_accounts`). **Daily trade count today: 0 of 3** (v3.78, fresh day — RKLB's two trades were yesterday, don't carry over).
- **Headlines/macro: today is the FOMC decision day.** Announcement 2:00pm ET, Chair Warsh's press conference 2:30pm — **after today's 11:00 close**, so it won't hit the actual trading window directly, but pre-market positioning already reflects it: a 25bp hike (to ~4.00%) is ~91-92% priced (Fed funds futures), and this is one of the quarterly meetings carrying an updated dot plot, so the vote count and projections likely matter more than the near-certain hike itself. 10-year yield still near its highest since 2023. Monday's Amodei AI-slowdown-safety call is still the backdrop for semis/AI-infra sentiment, though not this system's concern anymore (Core Ten only, no individual stocks).
- **Pre-market prices, the ten proxies — C1's actual gate surface (v3.76), none of this is a decision yet (RVOL/ORB can't complete before 9:45):** QQQ $707.83 (+0.47%) · SPY $759.44 (+0.27%) · SOXX $506.18 (+1.47%) · IWM $285.84 (+0.25%) · XLF $56.86 (+0.02%, ~20min-stale read) · XLE $65.22 (-1.07%, the one clearly negative proxy) · GLD $398.72 (+1.16%) · TLT $80.93 (+0.27%) · XBI $154.76 (+0.47%, ~20min-stale) · XRT $84.13 (+1.10%). Real early dispersion — SOXX/GLD/XRT firmest positive, XLE the lone negative — but pre-market thinness means none of it is trusted until the live 9:30-9:45 range forms.
- **Pre-market context on the twenty leveraged tickers (not gated, informational only, C4):** TQQQ $68.80/SQQQ $40.08 · UPRO $145.12/SPXU $35.80 · SOXL $106.64/SOXS $48.95 · TNA $63.18/TZA $43.99 · FAS $170.27/FAZ $33.37 (both ~20min-stale) · ERX $111.94-112.98/ERY $8.60 (ERX spread wide pre-market) · UGL $50.25/GLL $22.79 · TMF $29.38/TMV $44.61-44.72 · LABU $253.63/LABD $7.64 (both ~20min-stale) · RETL $7.40-7.54/SZK $19.43-24.38 (SZK's spread is extremely wide pre-market — thin, flag for liquidity at entry if its row ever gates bearish).
...
**9:45 — first live C1 run under v3.76-v3.78. Declined at the final placement re-check — no trade.** A1 re-confirmed: loss streak 2 of 3, flat, daily trade count 0 of 3. **Step 1 (RVOL)**: current volume (9:30-9:45 cumulative, minute bars) vs. 20-day average daily volume scaled to the same ~16-minute fraction of the trading day (no per-minute 20-day history available, so this is a same-time-of-day *approximation*, flagged plainly, not the literal minute-matched average) — only **SOXX cleared 1.5× (≈1.55×)**; QQQ (~0.94×) and IWM (~0.91×) ran hot on price but not on relative volume. **Step 2 (ORB)**: three proxies broke their own 9:30-9:45 range at the 13:46 read — QQQ, SOXX, IWM all above their opening highs — but only SOXX also cleared step 1, so it's the sole candidate into step 3 (nothing to rank against). **Direction: bullish (broke above) → candidate leg SOXL.** **B1 profile** (`tools/profile.py`, 29 daily bars): stop 5.37% (clear of the 7% ceiling, no blackout), target 8.06% (informational), stall threshold 0.35%. Affordable size at $455.66 settled cash ÷ $107.92 ask = 4 shares. `review_equity_order` came back clean, no alerts. **Final re-check before placing (C8/v3.50/v3.76) failed**: a fresh SOXX quote read $507.865/ask $507.98, both *below* its own $507.99 opening-range high — SOXX had fallen back inside its own range in the ~15 seconds between the gate-stack read and order placement. Per C8, this makes the row no longer eligible — **declined, no order placed.** No other proxy cleared both steps this checkpoint, so there was nothing to fall through to; passed per C9 rather than forcing a trade. A real, live example of exactly the failure mode this re-check exists to catch.

**9:50 — SOXL entered, first live trade of the new system.** A1 re-confirmed: loss streak 2 of 3, flat, daily trade count 0 of 3 pre-entry. **C1 re-run fresh**: SOXX again the sole survivor — RVOL ≈1.55× (still clearing 1.5×; QQQ ≈0.94×, SPY ≈0.58× ran hot on price but not relative volume), broke back above its own $507.99 opening-range high (live $508.14-508.33 across the checkpoint), direction bullish → SOXL. B1 profile unchanged from 9:45 (same day, same 29-session history): stop 5.37%, stall threshold 0.35%. Affordable size: floor($455.66 ÷ $108.05 ask) = 4 shares. `review_equity_order` clean, no alerts; SOXX re-verified above its OR high immediately before placing (508.14, C8's final check passed this time). **Filled 4 sh @ $107.8311 avg** (13:51:10 UTC / 9:51:10 ET), cost **$431.32** — better than the $108.40 marketable limit and the $108.05 ask read at review (price improvement, ~-0.20%). **Stop placed and confirmed resting**: stop_market sell 4 sh @ **$102.04** (5.37% below fill, `state: confirmed` verified via `get_equity_orders`). Next grid slot (9:55) is 5 min out — no entry+5 catch-up trigger needed (dormant under v3.64 cadence). **Daily trade count now 1 of 3** (v3.78). Intended exit: stop $102.04 (-5.37%) or a reversal below SOXX's own $507.99 opening-range high (the level that justified entry) at the next checkpoint if it fades back through it; target ~$116.51 (+8.06%, informational only) if the ratchet lets it run that far.

**9:55 management — first ratchet, stop raised.** `run_high` $107.8311 → **$108.29** (13:51-13:55 window). Live $108.275 > entry fill $107.8311 — in profit, ratchet applies (v3.63). No SOXL trade history yet for the Hindsight Rule (v3.75), multiplier stays 2. `candidate_stop = 108.29 × (1 − 2×0.35%) = $107.53`; live staleness guard clear (live $108.275 well above candidate). Cancelled the resting $102.04 stop (confirmed cancelled, no fill raced it), placed and confirmed resting: stop_market sell 4 sh @ **$107.53** (a large first raise, expected given the tight 0.35% stall threshold following price closely). Unrealized: +$1.78 (+0.41%). No B3 exit — SOXX still well above its $507.99 opening-range high, no reversal. Pre-commit: exit at the $107.53 stop if hit, or a reversal in SOXX back below its own $507.99 opening-range high at the next checkpoint.

**10:00 management — second ratchet, stop raised again.** `run_high` $108.29 → **$108.921** (13:55-14:00 window). Live $108.79 > entry fill — still in profit, ratchet applies. `candidate_stop = 108.921 × (1 − 2×0.35%) = $108.16`; live staleness guard clear. Cancelled the resting $107.53 stop (confirmed cancelled), placed and confirmed resting: stop_market sell 4 sh @ **$108.16**. Unrealized: +$3.84 (+0.89%). SOXX now $509.54, further above its $507.99 opening-range high — no reversal, no B3 exit. Pre-commit unchanged: the mechanical stop, or SOXX reversing back below $507.99.

**10:05 — stop found already triggered, SOXL closed, C12 mini-cycle declined.** `get_equity_positions` came back empty — the resting $108.16 stop had filled cleanly on its own at **$108.15, 10:01:45 ET** (`get_equity_orders`, `state: filled`), 4 minutes before this checkpoint discovered it. SOXX had peaked $509.54 then reversed hard back through $507.99. **Net: +$1.28, +0.296%, r=0.055** — a small, clean winner, not a loss; **resets the loss streak to 0 of 3** (E1 — a winner resets regardless of magnitude). Logged to `archive/trades.csv`. **Daily trade count stays 1 of 3** (exits don't add to the count, only entries). **C12 mini-cycle**: a profitable exit checks its own row first (v3.67 step 0) — SOXX read $507.98/ask $508.02 at the recheck, back *inside* its own opening range (507.98 < 507.99), no longer breaking bullish — step 0 fails, fall through to the full field. **T+0 re-run on all ten proxies**: SOXX still clears RVOL (~1.61×) but no longer clears ORB (inside its own range); QQQ clears ORB (709.42 > 708.81) but not RVOL (~0.71×); GLD broke *below* its own opening-range low (396.60 vs $397.92, candidate GLL) but not RVOL (~0.43×) — **zero proxies clear both steps, a correct no-trade.** Elapsed since fill is ~6 minutes; fill_time+10min lands at 10:11:45, after the regular 10:10 slot — letting 10:10 do double duty rather than arming a redundant ad hoc trigger, same precedent as prior sessions' close-checkpoint calls. Resume the standard grid at 10:10.

**10:10 — regular checkpoint (also doubling as the C12 fill+10min re-check), flat, no trade.** Four proxies broke their own opening range this read: QQQ (709.36 > 708.81, bull), SOXX (508.21 > 507.99, bull, recovered again), GLD (397.46 < 397.92, bear → GLL), XRT (83.70 > 83.61, bull → RETL, a fresh breakout). **None cleared RVOL (~42 min elapsed): QQQ ~0.64×, SOXX ~1.37× (down from 1.61× at 9:55 as volume growth slowed), GLD ~0.43×, XRT ~0.80×.** Step 1's fallback (rank all ten by raw cumulative volume, carry the top into step 2) was checked: **XLE ranks highest** (≈3.08M shares) but is still sitting inside its own $64.485-$65.14 range — fails step 2. **Zero proxies clear both steps — correct no-trade.** Daily trade count holds at 1 of 3, loss streak 0 of 3.
...
**10:25 — ERY entered via the raw-volume fallback, 2nd trade of the day.** QQQ, SOXX, XLE, and GLD all broke their ranges this read, but none cleared RVOL (~0.55×, ~1.11×, ~0.89×, ~0.41× respectively) — XLE's own 20-day average volume is naturally huge (energy sector), so its high raw volume doesn't translate to elevated *relative* volume, a real distinction the RVOL design is meant to catch. **Step 1's fallback triggered**: nothing cleared 1.5×, so ranked all ten by raw cumulative volume — **XLE topped the ranking again** (≈3.57M shares, a clear, stable lead over XLF/TLT/QQQ all day) — and XLE **also broke below its own $64.485 opening-range low** (live $64.40-64.44 across the read), clearing step 2 too. Direction bearish → candidate leg **ERY**. B1 profile (`tools/profile.py`, 29 daily bars): stop 2.50% (floor-clamped, well under the 7% ceiling), stall threshold 0.21%. Affordable size: floor($456.94 ÷ $8.81 ask) = 51 shares. `review_equity_order` clean; XLE re-verified still below its OR low immediately before placing ($64.40). **Filled 51 sh @ $8.81 avg** (14:27:12 UTC / 10:27:12 ET), cost **$449.31**. **Stop placed and confirmed resting**: stop_market sell 51 sh @ **$8.59** (2.50% below fill). **Daily trade count now 2 of 3** — at the recommended pace (v3.78); a 3rd would be discretionary from here. Intended exit: stop $8.59 or a reversal in XLE back above its own $64.485 opening-range low.

**10:30 management — no ratchet, live price dipped below entry fill.** `run_high` $8.81 → **$8.815** (14:27-14:30 window). Live $8.7902 < entry fill $8.81 — per v3.63, profit gate blocks any ratchet. Stop stays at $8.59, untouched. Unrealized: -$1.01 (-0.22%). XLE bounced back to $64.475-64.48, right at the edge of its $64.485 level but not yet back above it — no B3 exit, no named level broken. Pre-commit: exit at the $8.59 stop if hit, or XLE closing back above its own $64.485 level at the next checkpoint if the bounce continues.

**10:35 — pre-commit honored, ERY closed.** XLE read $64.49-64.50 — back *above* its own $64.485 opening-range low, the level named in the 10:30 pre-commit. Per B3 ("then honour it"), exited rather than waiting for the mechanical stop 2.28% further away: cancelled the resting $8.59 stop (confirmed cancelled, no fill raced it), sold **51 sh @ $8.79** (marketable limit at the bid, filled exactly there, 14:37:31 UTC / 10:37:31 ET). **Net: -$1.03, -0.227%, r=-0.091** — a real but small scratch loss; at E1's -1.0% threshold this does not extend the loss streak (stays 0 of 3). Logged to `archive/trades.csv`. **Daily trade count stays 2 of 3.** Flat, no resting orders. **C12 mini-cycle**: this was not a profitable exit, so v3.67's step 0 shortcut doesn't apply — falls straight to the full field. XLE is now back inside its own range (ERY itself excluded from re-entry today regardless, per v3.68). **T+0 re-run**: only QQQ still breaking (709.28 > 708.81) among the other nine, and it fails RVOL (~0.48×, decaying most of the morning); everything else sits inside its own range. Zero candidates — correct no-trade. Elapsed since fill is 0 min; fill_time+10min lands at 10:47:31, well after the regular 10:40 and 10:45 slots — letting those absorb any further re-check rather than arming a redundant ad hoc trigger. Resume the standard grid at 10:40. **Daily trade count holds at 2 of 3** — the 3rd entry today, if any, is discretionary per v3.78.

**10:40 — flat, no trade.** Only QQQ still breaking (709.29 > 708.81), RVOL still ~0.49×; everything else, including XLE (right at the edge of its own low), sits inside its own range. Correct no-trade.

**10:45 — flat, no trade, a genuine edge case flagged.** Five proxies broke this read: QQQ (bull, RVOL ~0.48×), SPY (bull, new breakout, RVOL ~0.36×), TLT (bull, new breakout, RVOL ~0.54×), XRT (bull, RVOL ~0.61×), and XLE (bear again, back below $64.485). None of the four bullish breakouts clear RVOL. **XLE still tops the raw-volume fallback ranking by a wide margin** (≈4.35M cumulative vs QQQ's 2.88M) and is breaking — but its candidate leg for this direction is **ERY, already excluded for the rest of today (v3.68)** since the 10:35 exit was a loss on that exact ticker. Rule as written only says "carry the top of the ranking into step 2" — it doesn't specify what happens when that top pick's direction points at an excluded ticker; treated conservatively as no-trade here rather than cascading to the next-ranked proxy, since nothing in C1/C12 authorizes that cascade. **Flagging for D4/governor review**, not proposing a fix from one occurrence — if XLE keeps winning the fallback ranking while ERY sits excluded, this is worth an explicit rule rather than an ad hoc judgment call each time.
...
**11:00 — close, day complete, Thursday's grid armed.** Flat, no position to close. **Wednesday 2026-09-16 final tally (D3): 2 trades, SOXL +$1.28 (+0.30%) and ERY -$1.03 (-0.23%), net +$0.25 on the day.** Loss streak 0 of 3, daily trade count finished at 2 of 3 (recommended pace, one under the hard cap). Total value **$455.91** (deposited capital $450.81 unchanged — realized P&L moved from $4.85 to $5.10, exactly offsetting the day's net gain). Floor $225.41, unchanged. **First full live day under the v3.71-v3.78 restructuring** (Core Ten universe closed to 10 proxy-gated rows, RVOL→ORB→ATR entry mechanism, 11:00 close, daily trade cap, lean cleanup) — validated end to end: a real gate-stack decline caught by the C8 final re-check (9:45), a clean entry/ratchet/stop-out cycle (SOXL), a pre-commit-honored exit (ERY), a C12 mini-cycle correctly finding no re-entry twice, and one genuine edge case flagged for review (raw-volume fallback pointing at an excluded ticker). **Thursday 2026-09-17's full 19-checkpoint chain armed** (9:00 → 9:30 → 9:45-10:55 management ×15 → 11:00 close+arming → 8:00pm backup), verified via `list_triggers` — all 19 present, no gaps or duplicates. Commit and push.

**13:31 ET, post-close — new deposit landed, floor recomputed.** Governor stated total deposited is now $500. Verified live: `total_value` jumped from the 11:00 close's $455.91 to **$505.07** (`get_portfolio`), a **$49.16 deposit** — `pending_deposits` reads $0, so it's already settled and spendable, matching `buying_power` = `cash` = $505.07 exactly. **Deposited capital recomputed** (E2's own formula, never cached): `$505.07 − $5.10 (all-time realized P&L, unchanged since the 11:00 close, no trades since) − $0 unrealized` = **$499.97**, matching the governor's stated $500 almost exactly (3-cent rounding drift, immaterial). **Floor moves from $225.41 to $250.00** (50% of the new deposited figure) — takes effect immediately, binding at Thursday's 9:00 checkpoint and every one after. **One anomaly noted, not yet resolved:** `get_accounts` reports this account's `unsettled_funds` as $880.88 — inconsistent with a $505.07 total value on a limited-margin account with no borrowing beyond its own cash (E2). `get_portfolio`'s numbers (the authoritative buying-power source per its own tool guide) are internally clean and consistent with the deposit math above, so this isn't blocking, but it's flagged for the next checkpoint to re-check rather than assumed stale.
