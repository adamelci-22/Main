...
**Policy version: 3.72.** Bump on every rule/threshold change; record it in the commit.
...
> **No position may be opened outside 9:45–11:15 (v3.72 — moved from 9:40 to 9:45, direct governor instruction 2026-09-16, so C1's Opening Range Breakout has a full 9:30–9:45 window to form before any entry decision).** 9:40 stops being a decision point at all — it's still a scheduled checkpoint, but purely for tracking the forming opening range (B6), no entry possible that early anymore. Multiple round trips per day, across different candidates, are still possible (limited margin, since 2026-08-20) — a fresh entry may be taken at **any** checkpoint while flat, not only 9:45. **A position that closes mid-day gets an accelerated re-check instead of waiting for the next grid slot — see C12.**
...
- **Entries are valid at any checkpoint from 9:45 through 11:15** (v3.72 — moved from 9:40 to give C1's Opening Range Breakout its full 9:30–9:45 window; v3.64's tightened 5-min cadence set the last checkpoint, and so the last entry opportunity, at 11:15) — no preferred-window distinction inside that range beyond C1's own ATR-expansion ranking. **11:30 itself is exit-only** — the close checkpoint doesn't take new entries, same as it didn't when it was the double-duty slot at 11:00 (v3.57).
