...
*Structural finding, first surfaced building today's list under D2 v3.29's 5-sectors-×-3-stocks structure: checked every group in E3's sector-proxy map, and only two — SMH (semis) and IBIT (crypto) — have ≥3 individual-stock members at all. Every other group (energy, gold, silver, materials, uranium, biotech, Korea, China, nat gas, broad index) is sector-ETF-only in the map, zero individual names. This isn't a today-specific gap — it's true of the map itself, so most days will structurally cap out well below 5 qualifying sectors regardless of how many real, catalyst-backed movers show up. Per D2's own escape valve ("if a sector can't support 3 real names, it doesn't get a slot — pick a different one that can"), today's list correctly has 2 sector slots, not 5. Flagged for the governor rather than silently forced or silently padded.*

*2 sectors qualified (of a 5-sector target), profiled fresh (`tools/profile.py`, 30 daily sessions each), ranked by `mfe_per_stop`:*
*— **SMH (semis)**, 7 candidates profiled — top 3: SMCX 0.755 · MUU 0.687 · TSMX 0.388 (also profiled, not selected: NVDX 0.362, AMDL 0.360, NVDL 0.337, AVGX 0.323 — AMDL, MUU and SMCX all hit the 7% stop cap, noise wider than the stop).*
*— **IBIT (crypto)**, 9 candidates profiled — top 3: BITX 0.876 · MSTX 0.839 · BITU 0.818 (also profiled, not selected: CONL 0.707, RIOT 0.685, ETHU 0.634, ETHT 0.608, MARA 0.495, CLSK 0.434).*
*C3 magnitude (+0.75%, informal, re-confirmed live at 9:40): semis group clears easily across the board (SMCX +5.05%, MUU +3.97%, TSMX +3.10% premarket). Crypto group is mixed — BITX/BITU/CONL/ETHU/ETHT all currently under +0.75% premarket, only RIOT/MARA/CLSK clear it; MSTX is negative. Selection above is by `mfe_per_stop` per C7/D2 (never by current-day price magnitude) — today's actual C3 clearance will be re-checked live at 9:40 regardless of this ranking.*
