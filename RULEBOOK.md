...
**Policy version: 3.34.** Bump on every rule/threshold change; record it in the commit.
...
*Premarket day changes (~9:03 ET, last non-reg trade vs. adjusted previous close): SMH +2.00%, SOXL +6.33% (3x, confirms the read) — semis clearly today's dominant mover. IBIT +0.25%, BITX/BITU/RIOT/MARA/CLSK/ETHU/ETHT all modest single-digit-tenths positive, MSTX/MSTR actually **negative** (-3.12%/-1.53%) — crypto has cooled sharply from Monday, no fresh catalyst found overnight. Other real movers seen: XBI +0.71%/LABU +2.65% (biotech), URA +2.65% (uranium), EWY +2.93% (Korea) — all lack any individual-stock members in the E3 map (see finding below). Energy/gold/silver/nat gas all red premarket (XLE -1.42%, GDX -0.86%, GLD -0.40%, SLV -1.51%, UNG -2.47%, USO -3.38%) — not viable longs this morning.*

*Structural finding, first surfaced building today's list under D2's 5-sectors-×-3-stocks structure: checked every group in E3's sector-proxy map, and only two — SMH (semis) and IBIT (crypto) — have ≥3 leveraged individual-stock members at all. Every other group (energy, gold, silver, materials, uranium, biotech, Korea, China, nat gas, broad index) is sector-ETF-only in the map, zero individual names. Governor's fix (v3.30): fill leveraged first, fall back to real plain (unleveraged) stocks from that sector — found live via the scanner's Sector filter or direct quotes on known constituents, never invented — when the leveraged bench runs out. Applied it live, same morning: added biotech (XBI/LABU) and uranium (URA) as two more qualified sectors using plain-stock individuals. Korea (EWY) was checked and declined — CPNG, the most liquid Korea ADR, is actually negative on the day, and the rest (KB/SHG/PKX/KT/WF) only manage +0.2% to +1.8%, too thin to call a real theme next to the other four's clean signals. Today's list: **4 of 5 target sectors**, not because the map was allowed to force a bad fit, but because a 5th genuine theme wasn't there this morning.*

*4 sectors qualified, profiled fresh (`tools/profile.py`, 30 daily sessions each), ranked by `mfe_per_stop`:*
*— **SMH (semis)**, leveraged, 7 candidates — top 3: SMCX 0.755 · MUU 0.687 · TSMX 0.388 (also profiled: NVDX 0.362, AMDL 0.360, NVDL 0.337, AVGX 0.323).*
*— **IBIT (crypto)**, leveraged, 9 candidates — top 3: BITX 0.876 · MSTX 0.839 · BITU 0.818 (also profiled: CONL 0.707, RIOT 0.685, ETHU 0.634, ETHT 0.608, MARA 0.495, CLSK 0.434).*
*— **XBI (biotech)**, plain-stock fallback, found via a live Sector=Healthcare scanner scan (103 matches >+3%, broad-based, not one outlier — real sector move) — top 3: CRSP 0.607 · NTLA 0.577 · MRNA 0.567.*
*— **URA (uranium)**, plain-stock fallback, found by checking known uranium miners directly (CCJ/DNN/UEC/UUUU/NXE) rather than the generic Basic Materials scan, which returned mostly unrelated small-caps — top 3: UUUU 1.072 · UEC 0.935 · DNN 0.903 (also profiled: CCJ 0.544).*

*C3 magnitude (+0.75%, informal, re-confirmed live at each entry check): semis cleared easily (SMCX +5.05%, MUU +3.97%, TSMX +3.10% premarket). Crypto mixed — BITX/BITU/CONL/ETHU/ETHT all under +0.75% premarket, only RIOT/MARA/CLSK clear it; MSTX negative. Biotech and uranium picks were screened live already above +2%, comfortably clearing. Selection above is by `mfe_per_stop` per C7/D2 (never by current-day price magnitude).*
...
*C12 mini-cycle (fill-timestamp baseline 10:04:10 ET): the armed T+10 trigger itself ran ~4 minutes late (scheduled 14:15:00Z, actually fired 14:18:48Z) — treated as a delayed/missed checkpoint per A2 and run manually rather than waited on further; the delayed firing landed on the same content afterward and was a non-event. Gate stack result: **crypto and semis both eliminated on re-check** — full-group Gate 2 ranking now puts MARA/CLSK/CONL on top in crypto (none of shortlisted BITX/MSTX/BITU make top 3) and AMD/NVDA ahead of TSMX in semis; SMCX and MUU both fail C10 (falling since the fill moment, unsurprising for the name just exited). **Biotech and uranium survive** — plain stocks, not C1/C2-gated, all six (CRSP/NTLA/MRNA, UUUU/UEC/DNN) clear C10 leg 1 and C3 magnitude; C11 computed rigorously from real minute bars, all six clear the 0.15 minimum comfortably (CRSP 0.293, NTLA 0.179, MRNA 0.169, UUUU 0.244, UEC 0.277, DNN 0.294). C10 legs 2/3 default-pass for all six per B6 (first live read today, no prior session_high/low history — flagged, not genuine clearance). **UUUU ranked #1 by `mfe_per_stop` (1.072) — entered 10:20 ET, 14sh @ $15.3499 avg, stop $14.95 (-2.58%) confirmed resting on the first attempt.***

*Today's watchlist: 16 names total (4 proxies + 12 individuals) — SMH/SMCX/MUU/TSMX, IBIT/BITX/MSTX/BITU, XBI/CRSP/NTLA/MRNA, URA/UUUU/UEC/DNN.*

*11:30 checkpoint: UUUU's run_high advanced to $15.995 (+4.20% from fill); correct new stop computed at $15.93 (velocity flag already permanent from 11:00) came back **rejected** — a second, distinct E6 failure mode from SMCX's silent cancellation, identified this time: price had already fallen to $15.87, below the stop being placed. Exited immediately rather than fight it; first marketable-limit attempt went stale against a moving bid and had to be re-priced once more before filling. Net +$6.86, +3.19%, r=+1.238 — another real win, again mechanically forced, not rule-triggered. Full detail in `archive/trades.csv`. Flat as of 11:32 ET; two round trips today, both cut short by the same underlying E6 issue rather than by an actual rule.*

*C12 mini-cycle #2 (fill-timestamp baseline 11:32:31 ET): nothing survived. SMH/XBI/MUU/CRSP/NTLA/MRNA all fail C10 leg 1 (falling since the fill moment). TSMX/BITX/MSTX/BITU fail Gate 2 (full-group ranking now favors AMD/NVDA and CLSK/RIOT/MARA). SMCX (the name just exited) clears leg 1 and Gate 2 but fails **C10 leg 2** — straight decline from $12.88 to $12.69 with no bounce at any point since the session high, a clean, well-reasoned decline rather than an arbitrary skip. UEC and DNN both clear C10 fully (UEC's is the standout: a real, confirmed bounce off a $13.25 low) but both fail C11 on a rigorously recomputed Efficiency Ratio — UEC 0.082, DNN 0.208, both well under the 0.25 minimum for the 11:00–12:00 window. Staying flat; grid resumes at the regular 12:00 slot.*

*12:00 noon-crossing checkpoint, flat: biotech and uranium individuals broadly rolled over vs. the last formal read (CRSP/NTLA/MRNA/UEC/DNN all fail C10 leg 1). TSMX/BITX/MSTX/BITU all fail Gate 2 again (RIOT/CLSK/MARA lead crypto's full group now). SMCX is again the sole Gate-2 survivor, and this time genuinely clears C10 too — a real bounce off a $12.64 low, giveback only ~5%. But it fails C11: ER recomputed rigorously at 0.1408, well under the 0.25 minimum for this hour. Staying flat; grid continues at 12:30.*

*Account: flat, $229.61 cash/buying power (verified live). Deposited capital ≈$201.48 (all-time realized P&L ≈+$28.13, recomputed fresh from trades.csv), floor ≈$100.74 — not binding. A1 clear: loss streak 0, weekly day-trade count 7 of 15 (SMCX + UUUU round trips). Next 9:00 research (Wednesday 2026-08-26) replaces this block wholesale per this section's own rule.*
