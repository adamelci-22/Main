...
- **Market-wide individual-stock scan (v3.53 D2 step 4), run live: 105 names cleared ≥0.75% day-change with the ≥10M average-volume liquidity floor.** New saved scan `4ceac364-d887-4afc-b3e1-7cfe991001e1` ("D2 market-wide individual-stock gainers"), asset type = STOCK only. **Judgment call flagged, not yet codified in D2**: applied a $50M market-cap floor on top of the scan's own filters before ranking — the raw 105 included several sub-$50M names with implausible-looking moves for their float (GIPR +48% on a $1.19M market cap, DAIC/GPUS/RITR/ZONE/MTNB/IVDA/GSUN/AIXI all similar single-digit-to-low-tens-of-millions market caps) that read as thin/shell-stock noise rather than genuine, tradeable movers. Excluded 10 names on that basis before ranking the rest by magnitude. Worth the governor formalizing this floor in D2 explicitly if it holds up — flagging rather than silently applying it going forward.
...
- Profiling (`tools/profile.py`) and affordability marking deferred to the 9:30/9:35 live re-checks per D2 step 7 — today's 25 plus the four commodity proxies (gold/silver/copper/uranium) are the shortlist those checkpoints will run against.
