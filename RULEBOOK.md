...
**Friday 2026-09-04, 9:00 research** (run on time, ~9:00–9:35 ET; first live day under v3.55 — entry+5 catch-up check added, 12:00 close reconfirmed unchanged):

- **A2 trigger hygiene: real backlog found and partially cleared.** `list_triggers` surfaced a large pile of fired-but-undeleted triggers going back through 9/1 (42 found and deleted this checkpoint — every Thu 9/3 and Wed 9/2 slot, plus Tue 9/1's 11:00–12:30 stretch). Paging stopped there for time (9:30 approaching) with `has_more` still true — **pre-9/1 stale triggers likely remain and should be purged in a dedicated pass**, not urgent (they don't refire on their own, just clutter `list_triggers` output) but flagged rather than silently left. A1 recomputed fresh from `archive/trades.csv`: **loss streak 0 of 3** (last governor clearance 2026-08-31; every trade since — GUSH 8/31, XOM 9/1, NUGT 9/2, both MSTX legs 9/3 — closed at or above the -1.0% streak threshold). Not blocked.
- **Headlines:** Fed's Waller hinted at a September rate hold in remarks reported 9/3 afternoon, and equities rallied into Thursday's close on that read (no fresh macro escalation overnight). No specific overnight geopolitical or macro shock identified for today's open. Today's real driver looks idiosyncratic/earnings-and-analyst-note-driven rather than macro-thematic (see scan below) — semis/memory (SNDK, MU, TSM, MRVL, KLAC) all cleared together, consistent with sector-wide strength around Oracle's cloud-growth framing and ongoing AI-infrastructure demand read-through, not a single macro catalyst.
- **Market-wide individual-stock scan (D2 step 4), run live on the existing saved scan** `4ceac364-d887-4afc-b3e1-7cfe991001e1` ("D2 market-wide individual-stock gainers": STOCK only, % change ≥0.75% day-change, ≥10M average-volume liquidity floor). **29 names cleared — every one is a viable candidate, no market-cap or share-price pre-filtering applied** (still-standing per the governor's 9/3 override; this scan's own filters never included one to begin with).
- **Today's 25-name individual-stock watchlist**, ranked by day-change magnitude, wrapper-carrying names given priority fill per D2 (TSM/MU both carry wrappers — TSMX/TSMU, MUU — and are kept over the 4 weakest plain movers that would otherwise have filled slots 22–25):
  1. NCPL +18.92% · 2. PLAG +11.44% · 3. MTNB +5.41% · 4. USAR +5.31% · 5. VBIO +4.29% · 6. MRNA +4.14% · 7. LIMN +3.78% · 8. CELZ +3.60% · 9. ORCL +2.81% · 10. ASTS +2.80% · 11. UMC +2.47% · 12. GPRO +2.18% · 13. KLAC +2.06% · 14. TTD +1.92% · 15. NOK +1.84% · 16. SNDK +1.35% · 17. NVTS +1.26% · 18. CYCU +1.21% · 19. AAOI +1.19% · 20. RDW +1.16% · 21. STLA +1.09% · 22. GLW +1.05% · 23. RKLB +1.00% · **24. TSM +0.91% (wrapper TSMX/TSMU exists — C4 rank-1 track, priority-filled)** · **25. MU +0.88% (wrapper MUU exists — C4 rank-1 track, priority-filled)**.
  **Bumped by wrapper priority** (would otherwise have filled 22–25 by pure magnitude, dropped instead): POET +0.96%, CRWV +0.88%, TME +0.85%, MRVL +0.82% — all four still mechanically qualified the scan and remain viable if re-checked live, just outside today's 25-slot cut.
- **Sub-$5 momentum sublist (still-standing addition per the governor's 9/3 request), ranked by premarket volume then %change** — all five already sit inside the 25 above, this is just the cheap/high-momentum lens on the same data: **GPRO** ($1.42, +2.18%, 181.1M vol) · **PLAG** ($0.72, +11.44%, 24.4M vol) · **NCPL** ($0.75, +18.92%, 8.6M vol) · **CYCU** ($3.63, +1.21%, 0.81M vol) · **MTNB** ($0.19, +5.41%, 0.81M vol).
- **Commodity check (D2 step 5): nothing qualifies today.** All six proxies flat-to-down premarket vs Thursday's close — XLE -0.70%, GLD -2.06%, SLV -2.87%, COPX -1.78%, URA -1.31%, XLB +0.02%. **No commodity goes on today's provisional list** — yesterday's gold/silver/copper/uranium strength has reversed overnight. Real C1 qualification would need a positive premarket read to even provisionally qualify; none of the six clear that bar.
...
**9:35 entry — ENTER USAR, 12 shares @ $18.8099 fill.** State check: flat, no resting orders (verified before placing).

- **C10 checked across the top movers using real 9:30→9:35 minute bars, not point quotes.** MRNA rolled over hard (9:30 baseline $152.11 → 9:35 $147.23, -3.2%) — declined outright, failed leg 1. NCPL/PLAG/MTNB/CELZ/LIMN (the sub-$5 momentum names) showed mostly thin or interpolated bars in the first five minutes — genuinely illiquid this early, no reliable signal to trade on, passed over rather than forced. **USAR and ASTS were the two clean reads**: both real volume throughout, both comfortably above their 9:30 baseline by 9:35 (USAR $18.54→$19.00+, ASTS $63.99→$65.28+), both essentially at session high (C10 leg 2 auto-satisfied, negligible giveback).
- **C11 computed directly off the six 9:30–9:35 one-minute closes for both finalists**: USAR ER 0.673, ASTS ER 0.953 — both far above the 9:35–10:24 bracket's 0.15 minimum, no chop concern either way.
- **C5 catalyst decided it.** USAR has a genuinely fresh, named, same-day, idiosyncratic catalyst: completed its $2.8B cash-and-stock acquisition of Serra Verde Group this morning (Benzinga/MT Newswires, both published ~7:30–8:10 AM ET today), creating one of the only integrated rare-earth-to-magnet platforms outside Asia — company-specific, not a sector ride. ASTS's most recent news was dated 9/3 (yesterday) — no fresh same-day driver found, just carryover momentum from Wednesday's rally. C4: neither carries a listed leveraged wrapper (E3) — both trade plain (rank-2).
- **Profiled both** (`tools/profile.py`, 46 daily sessions each): **USAR** stop 4.06%, target 6.10%, stall threshold 0.43%, **mfe_per_stop 0.708**. ASTS stop 4.82%, target 7.23%, stall threshold 0.46%, mfe_per_stop 0.636. **C7: USAR ranks higher on both the primary metric and the catalyst check** — selected over ASTS, no ranking contest needed once both cleared C1–C11.
- **Order execution**: reviewed at $18.89 ask, marketable limit $19.10, filled **$18.8099** (price improvement, $0.29/sh better than the limit). 12 shares (floor($245.46 buying power ÷ $19.10 live ask) = 12), total cost $225.72. **C10 re-verified live immediately before placement**: $18.925 (13:38:11 UTC), comfortably above the $18.5378 baseline. **Stop placed and confirmed resting**: stop_market, 12 sh, **$18.05 (−4.06%)**, confirmed via `get_equity_orders` (state=`confirmed`). Target 6.10% (~$19.96), `mfe_per_stop` 0.708. **v3.55 entry+5 catch-up check: not needed** — 9:40 already follows in exactly 5 minutes on the regular grid, so no ad hoc trigger armed (per C8's own carve-out). **Pre-commit for 9:40**: expect `bar_close` to hold at/above the trail; a close back below USAR's own 9:30 baseline ($18.5378) would be the falsified case for the "not currently falling" read that got it in.

**Real C4 miss caught immediately after entry, ~9:39 ET: E3's wrapper table was stale.** USAR has two real, tradeable 2X leveraged wrappers — **USGG** (Themes 2X Long USAR Daily ETF) and **USAX** (Tradr 2X Long USAR Daily ETF) — neither listed in E3, which hadn't been updated since v3.53's rewrite of the nine original single-stock wrappers. Per C4, the leveraged vehicle should have been preferred over plain stock. **Did not switch**: by the time this was caught, USAR had already pulled back to $18.29/$18.30 (from the $18.8099 fill) — closer to the $18.05 stop than to breakeven. Selling now to switch vehicles would realize a real ~$6 loss just to correct the instrument choice, then re-enter the leveraged version at a worse relative price — compounding the miss, not fixing it. **Kept the plain USAR position, managed on its own terms.** E3 corrected (USGG/USAX added), plus a standing instruction added there: verify with `search` before declaring "no wrapper exists," don't trust the table alone — it's necessarily incomplete since new leveraged products launch continuously.

**EXIT USAR, 12 sh @ $18.4101 — fourth occurrence of the AFRM/GUSH/NUGT ratchet-breach pattern, and the fastest one yet.** At the 9:40 checkpoint, B1b's since-entry pull showed `run_high` had touched **$18.985** within the very first minute after the 9:38:26 fill (13:38 UTC bar) — `candidate_stop = 18.985 × (1 − 2×0.43%) = $18.8215`, already **above** live price ($18.31–$18.41 by the time the checkpoint ran) before a resting stop could ever be placed at that level. Same mechanical shape as AFRM (8/28), GUSH (8/31), and NUGT (9/2), compressed into **under 2 minutes** instead of 8–18 — the fastest round trip of the four. **Notable: v3.55's entry+5 catch-up check (added earlier today for exactly this gap) correctly found no ad hoc trigger was needed, since 9:40 already follows 9:35 by exactly 5 minutes — but the actual reversal here happened inside the first minute, faster than even a 5-minute check could have caught.** v3.55 doesn't fully close this gap; worth naming plainly rather than claiming the fix solved something it didn't reach.
...
**10:00 management — ENTER KLAG, 9 shares @ $25.3099 fill.** State check: flat, no resting orders (verified before placing). The market genuinely broadened since 9:00 — the semis/memory complex (already on today's 25-name shortlist) is running much harder now than at open: KLAC +7.5% (vs +2.06% at 9:00), SNDK +5.8%, MU +4.2%, MRVL +3.8%, INTC +3.5%. Re-ran the existing D2 scan (same saved scan_id, not a new one) to confirm live — 58 names now clear the floor, up from 29 at 9:00.

- **GPRO also showed an extreme +23% intraday move, checked and ruled out.** No fresh same-day catalyst — its real driver (Markiplier stake disclosure + Starman Optical merger) is 3-4 days old (8/31-9/1 news), today is pure continuation/short-squeeze dynamics. Worse, its daily history is completely distorted by that event (volume spiked from a ~5M average to 220M/500M/280M/179M across the last four sessions) — any risk profile built on that data would be meaningless, so it couldn't even be properly sized. Passed over despite technically clearing C10 (+19.7% since 9:30) and C11 (ER 0.462).
- **KLAC chosen instead**: real mega-cap liquidity, clean steady uptrend (ER 0.667 since 9:30, well above the 0.15 floor), and falls under C5's tech/semis exception (no single named catalyst required — broad AI-infrastructure/memory sector strength is the driver, stated plainly rather than invented). C10: $185.96 live vs $176.815 9:30 baseline, comfortably above and still climbing.
- **Profiled KLAC** (`tools/profile.py`, 46 daily sessions): stop 3.43%, target 5.14% (mfe_to_target 3.31 — above 2.5, target effectively unreachable, noted per C7 rather than treated as disqualifying), stall threshold 0.23%, **mfe_per_stop 0.453**.
- **C4 wrapper check done properly this time**: searched live rather than trusting E3 alone, found **KLAG** (Themes 2X Long KLAC Daily ETF) — real, tradeable, spread $25.25/$25.32. Preferred over plain KLAC per C4 (rank-1, affordable): 9 sh KLAG vs 1 sh plain KLAC for the same capital. E3 updated with the new wrapper.
- **Order execution**: reviewed at $25.31 ask, marketable limit $25.60, filled **$25.3099** (price improvement). 9 shares (floor($240.66 buying power ÷ $25.60) = 9), total cost $227.79. **Stop placed and confirmed resting**: stop_market, 9 sh, **$24.44 (−3.43%, KLAC's own profiled stop_pct applied directly to the KLAG fill, same convention as MSTX/MSTR)**, confirmed via `get_equity_orders` (state=`confirmed`). **v3.55 entry+5 catch-up armed** (`trig_0165XgkuYmM832xbHFuA62jE`, fires ~10:08 ET) since the next regular checkpoint (10:10) is 7 minutes out. **Pre-commit for 10:10 (or the 10:08 catch-up, whichever comes first)**: expect `bar_close` to hold at/above the trail; a close back below KLAC's own 9:30 baseline ($176.815) would be the falsified case.

**10:10 management — EXIT KLAG, 9 sh @ $25.0813. Real process error caught mid-position, not just a ratchet breach.** State check: 9 sh KLAG, `shares_held_for_sells=9`, stop confirmed resting at $24.44 going in. The v3.55 entry+5 catch-up (armed for ~10:08 ET) did not fire as a separate notification — this regular 10:10 slot covered the same ground.

- **The $24.44 stop was wrong from the start.** It used KLAC's profile (stop 3.43%, stall 0.23%) applied directly to KLAG's own price — B1's stated method (profile the exact traded instrument) was skipped. KLAG's own daily bars (46 sessions, pulled fresh at this checkpoint) show real stop 6.77%, stall 0.35% — roughly double, consistent with a 2x-leveraged, thinner-volume ETF genuinely swinging more than its underlying, not just tracking it cleanly. **B1 corrected** with a standing rule against this.
- B1b since-entry pull: `run_high` never advanced past the $25.3099 fill — the position never made a new high. Recomputed `candidate_stop` with the corrected stall_threshold: `25.3099 × (1 − 2×0.35%) = $25.132` — at first this sat right at live price ($25.03–$25.14, inside normal bid-ask noise), then price slipped further and the breach became unambiguous.
- **KLAC (the underlying) never reversed** — held +7.2% on the day throughout the entire hold. This was a leveraged-wrapper noise/thin-liquidity issue amplified by the wrong (too-tight) discount band, not a real thesis failure.
- Cancelled the $24.44 stop (verified `cancelled`), watched the corrected candidate level get clearly breached rather than force a stop placement already at/above market, exited via marketable limit at $24.90, filled better at **$25.0813** (price improvement). Net: **-$2.06, -0.90% on the position, r=-0.133** (computed against the corrected $23.60 initial stop, not the wrong one actually placed). True MAE/MFE: MFE ~0% (no new high made), MAE 1.40% ($24.956, 14:12 UTC). Logged to `archive/trades.csv`, `exit_reason=trail_breached_before_placement`. **Loss streak: 0 of 3** — per E1 (v3.49), -0.90% is a scratch *above* the -1.0% threshold, so it resets the streak rather than building it (recomputed fresh from `trades.csv`: USAR's -2.13% at 9:40 built it to 1, this one resets it back to 0).
...
**10:30 management — ENTER AXTI, 3 shares @ $60.5601 fill.** State check: flat, no resting orders (verified before placing). GPRO continued accelerating (+39% now, was +23% at 10:00) — still no fresh same-day catalyst, still avoided for the same reasons as before; the continued avoidance is validated, not a missed opportunity, since nothing changed about why it was declined. Re-ran the scan: SNDK re-accelerated to +7.0%, MRVL and AXTI emerged as new strong movers (+4.5%, +7.4%).

- **C10/C11 checked with checkpoint-cadence sampling (not raw 1-minute bars, closer to B6's actual design) since 9:30**: SNDK (ER 0.887), MRVL (ER 0.847), AXTI (ER 0.897) — all three showed real clean trends, much cleaner than 1-minute noise would suggest. **SNDK ruled out on affordability alone** — ~$1670/share, floor($238.60 ÷ ask) = 0 shares, unaffordable regardless of rank, named per C7's own requirement.
- **Profiled MRVL and AXTI** (`tools/profile.py`, 46 daily sessions each): MRVL stop 3.95%, mfe_per_stop 0.531. **AXTI stop 7.00% (at the cap, noise wider than the stop, flagged not disqualifying), mfe_per_stop 0.672** — ranks higher, chosen.
- **C4 wrapper check**: both MRVL (MRVU, MRVX) and AXTI (AXTC) have real wrappers. **AXTC declined on liquidity grounds** — many days under 5,000 shares (some zero-volume), and live spread showed bid $13.73/ask $13.80 vs last trade $13.36, a stale/wide market. Per today's KLAG lesson (profile and trust the actual instrument, not just its existence), an illiquid wrapper isn't a real affordable option even when nominally tradeable — **traded plain AXTI instead** (C4 fallback), tight live spread ($60.49/$60.60).
- **Order execution**: reviewed at $60.60 ask, marketable limit $61.20, filled **$60.5601** (price improvement). 3 shares (floor($238.60 ÷ $61.20) = 3), total cost $181.68. **Stop placed and confirmed resting**: stop_market, 3 sh, **$56.32 (−7.00%)**, confirmed via `get_equity_orders` (state=`confirmed`). **v3.55 entry+5 catch-up armed** (`trig_01563ZmKH8yWbZsFJVubg7S6`, fires ~10:37 ET) since the next regular checkpoint (10:40) is ~7.5 minutes out. **Pre-commit for 10:40 (or the 10:37 catch-up)**: expect `bar_close` to hold at/above the trail; a close back below AXTI's own 9:30 baseline ($56.5825) would be the falsified case.

**v3.55 entry+5 catch-up — HOLD AXTI, stop ratcheted up.** Fired at 14:39:39 UTC (slightly delayed vs the ~14:37 target, still well ahead of the 10:40 regular slot). State check: 3 sh AXTI, `shares_held_for_sells=3`, stop confirmed resting at $56.32 going in. B1b since-entry pull (14:32–14:40 UTC): `bar_high` reached **$60.8499** (14:32 bar, essentially at the fill itself), then a fresh live quote showed **$60.8868** (14:40:25 UTC) — a new high beyond any bar. `run_high = max($60.5601 fill, $60.8499 bar_high, $60.8868 live) = $60.8868`.

- `candidate_stop = 60.8868 × (1 − 2×0.71%) = 60.8868 × 0.9858 = $60.0217`, rounded to **$60.02**. `new_stop = max($56.32, $60.02) = $60.02` — a large real move (+6.5%). Cancelled the $56.32 stop (verified `cancelled`), re-checked live price ($60.80/$60.89, safely clear), placed and verified resting: stop_market, 3 sh, **$60.02**, confirmed via `get_equity_orders` (state=`confirmed`).
...
**10:50 management — HOLD AXTI, stop unchanged.** State check: 3 sh AXTI, `shares_held_for_sells=3`, stop confirmed resting at $60.02 going in. B1b since-10:40 pull (14:40–14:50 UTC): no new high — `bar_high` topped out at $61.23 (14:40 bar, unchanged from the prior checkpoint), price has been consolidating $60.42–$61.06 since.

- `run_high` stays $61.23, `candidate_stop` stays $60.3605 — still only 0.57% above the current $60.02 stop, below `min_stop_move_pct` (1.00%). Left unchanged per B2. Verified via `get_equity_orders` that the $60.02 stop is still resting, state=`confirmed`.
- B3 exits checked: no reversal (AXTI still well above its 9:30 baseline, consolidating not breaking down), no R/R flip, no event, not near close. **Hold.**
- **Pre-commit for 11:00**: the $60.02 stop is still the live falsifiable line — a close back through it is the exit.
