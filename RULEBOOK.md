...
**Tuesday 2026-09-08, pre-market funding event** — reported by the governor ahead of today's 9:00 research, verified against the broker rather than taken on faith: `get_portfolio` confirms **$250 deposited**, reconciles cleanly against Friday's close — $242.46 (post-AXTI-exit, +$3.86 on 3 sh) + $250 = **$492.46 buying power/cash**, exact match. `get_accounts` flagged `unsettled_funds: $632.19` on this account, which does **not** reconcile against anything in the trade history (Friday's AXTI sale was only ~$185.54) — noted as an unexplained anomaly, not silently trusted; `buying_power` is the documented authoritative spendable figure and is what's used below. **Deposited capital recomputed**: `total_value ($492.46) − all-time realized P&L ($41.62, get_realized_pnl span=all, 25 closing trades) − unrealized P&L ($0, flat)` = **$450.84** (up from ~$201 pre-funding). **New floor: $225.42** (50% of deposited, E2) — not binding, just recorded per the formula. This will be re-verified fresh at today's actual 9:00 research per D2 step 6, same as every day; recorded now per D3's "report immediately: a balance change indicating funding."
...
**EXIT AXTI, 3 sh @ $61.845 — governor manual exit, off-cycle.** Cancelled the resting $60.02 stop at 15:23:01 UTC and sold via market order, filled 15:24:00 UTC — a real win, and better than the resting stop would have captured, since price kept running after the 11:00 checkpoint's ratchet all the way to a session high of $62.17 before the governor closed it. Net: **+$3.85, +2.12% on the position, r=+0.303**. True MAE/MFE for the full ~51.5-minute hold: MAE 0.59% ($60.20, 14:34 UTC, before any ratchet), MFE 2.66% ($62.17, 15:14–15:19 UTC). Logged to `archive/trades.csv`, `exit_reason=governor_manual_exit`.

**OPERATIONAL GAP — flagged per D3.** This session was disconnected/reconnecting through the 11:15, 11:30, and 11:45 checkpoints — they queued as notifications but were not processed live. No new position was opened in that window (flat since the governor's manual exit at 11:24 ET), so no P&L impact, but the C12 mini-cycle that should have followed the AXTI exit never ran — a real missed re-entry check, same shape as Thursday's gap, not a new failure mode.

**12:00 close — flat, nothing to pin.** State check: `get_equity_positions` empty, no resting orders. B2's stop-pin mechanism has nothing to act on since the position was already closed manually 36 minutes before this checkpoint.

**D3 report — Friday 9/4 summary.** Three trades today, two small losses offset by one real win: entry #1 USAR 9:40 $18.8099 → stop-out 9:50:59 $18.4101 (-$4.80, -0.526R, the fourth AFRM/GUSH/NUGT-shape ratchet breach, fastest yet); entry #2 KLAG (C4 leveraged pick on KLAC) 10:03 $25.3099 → exit 10:16 $25.0813 (-$2.06, -0.133R against the corrected stop — this trade's real finding was a B1 profiling error, now fixed, not a bad signal); entry #3 AXTI 10:32 $60.5601 → governor manual exit 11:24 $61.845 (**+$3.85, +0.303R**). **Net day: -$2.83 (roughly flat), account value $242.46** (down slightly from ~$245.46 at Thursday's close, essentially round-tripping two small process-driven losses against one real win). Loss streak: 0 of 3 — both losing trades landed under the -1.0% streak threshold (E1), never built past 0. **Two operational gaps today** (post-USAR-exit and post-AXTI-exit C12 windows both went unprocessed live) — same shape as Thursday's, worth real attention if it recurs a third time. **Real process fixes made live**: v3.55 (entry+5 catch-up check), v3.56 (entry moved back to 9:40 per governor instruction), B1/E3 corrected for wrapper-profiling and wrapper-liquidity mistakes found and fixed the same day they occurred.

**Monday 9/7 is Labor Day — a market holiday, verified against the calendar rather than assumed (D1's own instruction).** Skipping it: **Tuesday 9/8's full checkpoint chain armed instead**, per D1 (primary arming) — Friday would normally arm Monday, but a holiday in between shifts the target to the next real trading day. New v3.56 grid: 9:00 research through 12:00 close plus the 8:00pm backup, entry at 9:40 (not 9:35), first management at 9:45.
