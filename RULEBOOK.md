...
**Thursday 2026-09-10, 9:00 research** (first live day under v3.59/v3.60 — entry-stop grace period and the 11:30 close/extended-management-window both now in force):

- **A1 loss streak recomputed fresh from `archive/trades.csv`: 0 of 3.** Not blocked. **Position: flat, no resting orders** (`get_equity_positions`, verified). Buying power **$487.33**, no pending deposits — up from $486.29 pre-trade Wednesday, reflecting Wednesday's own +$1.25 realized (no overnight funding activity).
- **Headlines/macro: real, risk-off, rates-driven — a different character than Wednesday's geopolitical/oil story.** Precious metals and copper selling off hard premarket on rising Fed rate-hike bets ahead of this morning's Core PPI and jobless-claims releases, with the Fed's own meeting next week the real focus. SLV **-4.08%**, GLD -1.17%, COPX **-6.00%**, URA -2.83% premarket — all four clearly negative. XLE the lone holdout, **+1.46%**, tracking crude staying firm. XLB flat (-0.14%). Broader tape mixed-to-soft: SPY -0.53%, QQQ -1.32% premarket (later index-level reporting showed a more mixed open — DIA +0.21%, SPY +0.13% — so treat the early premarket read as provisional, formal reads happen live at 9:30/9:40 per usual). Macy's, Adobe, Oracle, RH report earnings today; no results out yet as of 9:00.
- **Market-wide individual-stock scan (D2 step 4), run live on the existing saved scan** `4ceac364-d887-4afc-b3e1-7cfe991001e1`. **Only 21 names cleared** — thinner than Wednesday's 36-at-open, consistent with the risk-off tone. All 21 fit within the 25-slot cap, no cut needed.
- **Today's watchlist, ranked by day-change magnitude:**
  1. SNYR +51.63% · 2. GSUN +8.52% · 3. VG +3.30% · 4. KOS +3.14% · 5. LIMN +2.87% · 6. BTE +2.61% · 7. RIG +2.19% · 8. CHOW +2.01% · 9. XOM +1.88% · 10. DVN +1.86% · 11. MMA +1.77% · 12. HAL +1.67% · 13. SLB +1.42% · 14. T +1.13% · 15. KO +1.08% · 16. PBR +1.05% · 17. KMI +1.02% · 18. VZ +0.98% · 19. CELZ +0.96% · 20. NCPL +0.91% · 21. WMT +0.84%.
  **SNYR flagged heavily as suspect** despite topping the list: real, dated, same-day catalyst found (Benzinga: "Synergy CHC recently announced it filed for Chapter 11 Bankruptcy") but that's exactly the kind of news that should crash a stock, not pop a $1.2M-market-cap microcap 50%+ — reads as speculative/delisting-candidate froth, not a genuine tradeable move. Kept on the list per D2 (real filtering happens at C5/C10/C11) but expect it to fail on inspection at entry.
  **Real, coherent theme underneath the rest of the list**: the energy complex is moving together (XOM, DVN, HAL, SLB, PBR, KMI, RIG, BTE, KOS all up), consistent with XLE's own premarket strength and crude holding near $100 — same complex as Wednesday, still live. A smaller defensive/value cluster (KO, T, VZ, WMT) is also up, consistent with the broader risk-off rotation out of growth/metals.
  No commodity or individual-stock name here carries a known leveraged wrapper from E3's table — no priority-fill reordering needed today.
- **Commodity check (D2 step 5): only XLE positive premarket** (+1.46%) of the six group proxies — SLV/GLD/COPX/URA all negative on the rate-hike story above, XLB flat/marginal-negative. Provisional only; real qualification still needs C1's 9:30→9:40 test live.
- **Deposited capital recomputed**: `total_value ($487.33) − all-time realized P&L ($36.50, get_realized_pnl) − unrealized P&L ($0)` = **$450.83**. **Floor: $225.42** (50% of deposited) — unchanged from Wednesday.
- Profiling (`tools/profile.py`) and affordability marking deferred to the 9:30/9:40 live checks per D2 step 7, per usual. Full 21-name shortlist is live via `run_scan` on the same saved scan id — not re-pasted here; the ranked table above is the complete individual-stock candidate set for today, plus the one provisionally-qualified commodity (XLE).

**9:30 observation (A1 · C1 Gate-1 baseline)**: git verified in sync. A1 re-checked fresh: loss streak still **0 of 3**, flat, no resting orders, buying power **$487.33**, unchanged. C1 9:30 baseline recorded for XLE (the only provisionally-qualified commodity) and the other five for completeness:
  - **XLE +1.04%** ($65.99 vs $65.31) — still the lone positive, consistent with premarket
  - SLV -4.10% ($58.2301 vs $60.72), GLD -1.05% ($399.12 vs $403.35), COPX -6.41% ($89.16 vs $95.27), XLB -1.46% ($50.64 vs $51.39, deepened from premarket's -0.14%), URA -3.28% ($45.325 vs $46.86) — all five stay negative/excluded, consistent with or worse than premarket
  No new scan run (per C1). Next: 9:40 entry checkpoint — re-confirm XLE's C1 legs live, run C3/C10/C11/C4 on the individual-stock watchlist.

**9:40 entry — no candidate cleared, staying flat.** **XLE failed C1 outright**: 9:30 reading was +1.04%, 9:40 dropped to +0.07% — well below the 9:30 baseline, fails leg 3 hard. No commodity trade. **Live re-scan broadened sharply** (29 names cleared, up from 21 at 9:00) — AAPL (+1.75% underlying) stood out with a real, dated catalyst (Tuesday's "Surprise and Shine" event, foldable iPhone Duo unveiled, stock recovering today after initial skepticism). **C4 wrapper search found AAPU (Direxion 2x), AAPB (GraniteShares 2x), AAPE (Leverage Shares 2x)** — AAPU picked as clearly the most liquid (800K–4.7M daily volume vs AAPB's 13K–121K and AAPE's 2.9K–65K). Profiled (mfe_per_stop 0.645). **C10 was marginal from the start** (baseline $41.2233 at 9:30, live read $41.51 barely cleared leg 2's bounce threshold of $41.489) and **broke down before an order could be placed**: a follow-up live quote 90 seconds later showed AAPU back down to $40.9743 — below its own 9:30 baseline, leg 1 now failing outright. Declined, same discipline as yesterday's MVLL catch — the marginal read was real chop, not a clean signal. **Space/satellite cluster (RDW, ASTS, RKLB, SPCX, ONDS) accelerated hard through the checkpoint** (RDW +4.8%, ASTS +2.8%, RKLB +2.5%, SPCX +2.1%, all moving together — real breadth) but **no dated same-day catalyst found** on a direct check of RDW's own news (all hits dated back to early-August Q2 earnings) — declined per C5's "a catalyst you can name" requirement; this sector doesn't get tech/semis' catalyst exception. No other candidate combined real magnitude with a verified catalyst. **Staying flat — no trade this checkpoint.** Next: 9:45 management/re-entry checkpoint.

**9:45 management (flat, fresh entry) — AAPU entered.** Rechecked AAPU: live $41.46, session_low had dropped further to $40.62 (13:42), giving real cushion this time — leg 1 clears cleanly ($41.46 > $41.2233 baseline), leg 2's bounce threshold ($40.7175) clears easily off the checkpoint's own bar_high ($41.495), giveback only 32% (leg 3 pass). Not the marginal read from 9:40 — a real recovery. C11: still under 20min of logged history for this candidate — default-pass, flagged. Catalyst unchanged (Apple event recovery, C5). C4: AAPU remains the most liquid of the three real wrappers found. **Entry executed**: reviewed (live $41.46, re-verified >baseline), marketable limit buy 11 sh @ $41.60, **filled 11 @ $41.45 avg** (9:46:43 ET), cost **$455.95**, slippage **-0.10%** (filled below reviewed ask). **Stop placed and confirmed resting**: stop_market sell 11 sh @ **$40.41** (2.50% below fill). Next regular slot (9:50) is <5min out — no entry+5 catch-up needed (C8). **v3.59 note**: this entry's post-entry checkpoint 1 (no ratchet) will be the 9:50 slot; checkpoint 2 (half-rate) will be 10:00. Intended exit: stop $40.41 (-2.50%) or target ~$43.00 (+3.75%), ratcheting up only through 11:30 close (v3.60).

**9:50 management — post-entry checkpoint 1, no ratchet (v3.59).** Stop never touched. Minute bars since fill (13:46–13:49): `bar_high` $41.5999, `bar_low` $41.2406, `bar_close` $41.320. `run_high` $41.45 → **$41.5999** (tracked as usual, B1b). **Per v3.59, checkpoint 1 applies no ratchet — stop stays at $40.41 unconditionally**, regardless of `run_high`'s advance. Unrealized: -$1.43 (-0.31% from fill, live price pulled back slightly off its post-entry high). No B3 exit. **Pre-commit for 10:00**: exit fires on its own if live price touches $40.41 before then; otherwise checkpoint 2 (half-rate ratchet, v3.59) applies at 10:00.

**10:00 management — post-entry checkpoint 2, half-rate ratchet (v3.59) — real breach, direct exit.** Stop never touched by live price (still resting at $40.41 the whole window). Minute bars since 9:50 (13:50–13:59): `bar_high` $41.40, `bar_low` $40.7629, `bar_close` $41.010 — `run_high` unchanged at **$41.5999** (no new high since 9:47, one minute post-fill). `candidate_stop = $41.5999 × (1 − 0.24%) = $41.50` (half rate per v3.59). **Live quote checked first: $41.01 — already below the $41.50 candidate, a real breach**, same stale-candidate pattern as the standing E6 entry (7th instance, now with v3.59 in force). Cancelled the untouched $40.41 stop (confirmed cancelled) and **exited directly via marketable limit**: sell 11 sh @ $40.85 limit, filled **11 @ $40.921 avg** (10:01:12 ET). True MAE/MFE over the full ~14-minute hold: true high $41.5999 (9:47, MFE +0.36%), true low $40.7629 (9:56, MAE -1.66%) — never recovered above its first-minute spike, a fade from the open. **Net: -$5.82, -1.28%, r=-0.510 — a real loss.** Logged to `archive/trades.csv` and E6 (v3.59 solved the fast-pullback case it was built for; this is a different shape — `run_high` stalling immediately and never returning — flagged, no further change made off one trade). Position confirmed flat, no resting orders.

**10:10 — C12 mini-cycle (elapsed ≥10min since the 10:01:12 fill, full gate stack run immediately per step 2) — no re-entry.** Baseline for every candidate = price at the fill instant (10:01:12). **Commodities: all six now negative** — XLE itself flipped to **-1.05%** (was +1.04% at 9:30), joining SLV/GLD/COPX/XLB/URA. No commodity trade. **AAPL/AAPU: settled into flat chop since the exit** — fill-time baseline ~$41.02, live $41.06, barely positive but the path since (session_high $41.26 at 10:06, session_low $40.89 at 10:11) is genuine back-and-forth with no net progress, exactly what C11 exists to catch. Declined re-entry into the same instrument that just produced a real loss minutes ago on a materially similar read — not reactive, the fresh data itself shows chop, not a trend. **RDW/space cluster still accelerating (RDW now +4.30%) but still no dated same-day catalyst** on re-check — stays declined per C5. **No re-entry — staying flat.** Regular grid resumes at 10:20.

**10:20 management — no entry, staying flat.** Commodities still all six negative (XLE -1.13%, SLV -3.88%, GLD -0.78%, COPX -6.52%, XLB -1.15%, URA -2.86%). Individual-stock tape broadened further (42 names now) but nothing new clears both magnitude and catalyst: AAPL steady (+1.38%, same story), RDW still climbing (+4.02%) with still no dated catalyst found, **GPRO jumped to +14.42%** (checked fresh) but its only real news is a 4-day-old "AI pivot" story already flagged by an analyst as unsupported by fundamentals — same speculative-microcap shape as SNYR/BIAF, declined. No trade. Regular grid resumes at 10:30.
