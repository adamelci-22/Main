...
| Candidate already at its 2-entry daily cap, or `rvol_decay_pct` ≥30% against its first entry today | C12/v3.83, D4/v3.85 |
...
**v3.83 — per-instrument entry cap, direct governor instruction, 2026-09-23.** No single instrument (of the twenty leveraged tickers) may be entered more than **twice** in one trading day, full stop — not a judgement call, same tier as the 4-trade absolute ban above. This binds regardless of *how* the second re-entry attempt would otherwise be reached: C12 step 0's same-leg shortcut, C12 step 1's full field re-run, or v3.80's discretionary 3rd-entry path all defer to it. Once an instrument records its 2nd entry fill today, it's excluded from candidacy for the rest of the day — joining v3.68's real-loss exclusion in effect, though for an unrelated reason (entry-count exhaustion, not a loss; an instrument can hit this cap on two clean wins). Combined with the existing 3-trade daily cap and v3.80's 3rd-entry restriction, this means **at most 2 distinct instruments get traded in a day, and at most 1 total re-entry happens across the whole session** — 2 entries into 2 different names, with only one of those two ever re-entered once. Prompted directly by 2026-09-23: SOXS was entered three separate times that day (9:45, a full-field C12 re-run landing on it again at 10:05, then C12 step 0's same-leg shortcut at 10:30) — the first two won cleanly, but the third, on a visibly cooling RVOL signal (see D4's `rvol_decay_pct` rule, v3.85), lost -2.611% and turned the day's net on that signal negative. This closes the gap where nothing previously stopped the same name from supplying more than 2 of the day's 3 entries.
...
0. **A profitable exit checks its own instrument first — a stop is a pause to reassess, not a verdict (v3.67).** If the exit's `pnl_pct_position` was positive, before anything else in this mini-cycle, run C1's three steps (RVOL/ORB/ATR) on that row's proxy, alone, checking specifically whether it's still breaking the same direction (long or inverse) as the leg that just closed. Still clearing all three, same direction — still a valid breakout, not reversed — **re-enter that same leg directly**, sized fresh off current settled cash (C8), without waiting to rank it against the other nine proxies. Only fall through to step 1 below (the full field, freshly re-run) if the same row no longer qualifies in that direction, **the instrument is already at its 2-entry daily cap (v3.83)**, **or `rvol_decay_pct` against that instrument's first entry today is ≥30% (v3.85)** — the shortcut never re-enters a name for a 3rd time or on a meaningfully cooled signal, regardless of how clean the breakout still looks.

**A losing exit's own instrument is excluded from re-entry for the rest of the trading day, full stop (v3.68).** Not merely denied step 0's shortcut — removed outright from the candidate pool for every C1 run today, this mini-cycle's and any later one's, even if its row's proxy would otherwise clear RVOL/ORB/ATR cleanly in that direction. Applies symmetrically whichever of the twenty tickers it was. **The same full exclusion applies to any instrument already at its 2-entry daily cap (v3.83)**, for the unrelated reason of entry-count exhaustion rather than a loss, **and to any instrument whose `rvol_decay_pct` (D4) against its first entry today has reached 30% or more (v3.85, direct governor instruction, 2026-09-23)** — a decayed signal is excluded the same as a losing one or an exhausted one, whichever candidate-pool check catches it first. All three exclusions feed the same check, just triggered by different conditions. Compute `rvol_decay_pct` fresh at the moment of any would-be re-entry — C12 step 0, C12 step 1 landing on an already-traded name, or v3.80's discretionary 3rd-entry path alike — using the first entry's already-logged RVOL (E5) against a fresh C1 step 1 read; ≥30% blocks the re-entry outright, state the number either way. Threshold origin, rationale, and recalibration process: D4.
...
**RVOL decay metric on same-day re-entries (v3.84, refining v3.81, direct governor instruction, 2026-09-23).** v3.83 caps any single instrument to at most one re-entry per day — so there is exactly one well-defined comparison to make, not an ambiguous multi-checkpoint "trend" to eyeball. At the moment of that one re-entry (whether reached via C12 step 0, C12's full field re-run landing on an already-traded name, or v3.80's discretionary 3rd-entry path), compute:

```
rvol_decay_pct = (RVOL_first_entry − RVOL_reentry) ÷ RVOL_first_entry × 100
```

— `RVOL_first_entry` is the RVOL reading already logged in today's E5 at that instrument's first entry (no new pull needed, just read it back); `RVOL_reentry` is the fresh RVOL computed at this re-entry checkpoint, same as any other C1 step 1 read. Positive = RVOL has cooled since the first entry; negative = it's actually stronger now. Worked example, 2026-09-23's own 3rd SOXS trade: `(3.26 − 2.18) ÷ 3.26 × 100 = 33.1%` decay, on the day's only real loss.

**v3.85 — decay ceiling, direct governor instruction, 2026-09-23.** `rvol_decay_pct ≥ 30%` now blocks the re-entry outright (C12, A1) — the informational-only framing above is superseded; see C12's exclusion paragraph for the mechanics. Set below the 33.1% actually observed on the trade that prompted this metric, meaning it would have blocked that specific trade — a deliberate choice, not a coincidence, but still a staged hypothesis from a sample of one. **State `rvol_decay_pct` in the entry report every time a re-entry is attempted, whether it clears 30% and proceeds or blocks and doesn't** — a blocked attempt is exactly as informative to D4's improvement loop as an allowed one, don't skip logging it because nothing got traded. If the 30% line proves too tight (blocking re-entries that would have won) or too loose (letting through more losers) once real instances accumulate, that's D4's job to catch and propose adjusting — not a call to make from this one trade either direction.
...
**Thursday 2026-09-24, 9:00 research — first live day under v3.83/v3.84/v3.85.** Wednesday's full session (3 trades, all SOXS off the same SOXX signal: a scratch -$3.17, a win +$9.73, a real loss -$12.72; net -$6.16, hit the 3-of-3 daily cap, triggered the new early-shutdown rule) archived in `archive/trades.csv` and git history — not restated here. Three rule changes landed yesterday (v3.81-v3.83) plus two more overnight in the cleanup/formalization pass (v3.84's `rvol_decay_pct` formula, v3.85's 30% decay ceiling) — policy version now **3.85**, all five live from this checkpoint forward.

- **A1 checks: loss streak 1 of 3** (Wednesday's 3rd SOXS trade, -2.611%, is the most recent close and a real loss under E1; the 2nd trade was a real win, so the streak doesn't extend past 1). **Position: flat, no resting orders** (`get_equity_positions` empty, `get_equity_orders` confirmed state empty). **Daily trade count today: 0 of 3** (fresh day). **Per-instrument 2-entry cap (v3.83): no instrument has been touched today yet — full field open.**
- **Account state:** total value/cash/buying power **$510.80** (`get_portfolio`), unchanged from yesterday's close. **Deposited capital recomputed**: `$510.80 − $10.88 (all-time realized P&L, get_realized_pnl) − $0 unrealized` = **$499.92**, unchanged from yesterday. **Floor: $249.96** (50% of deposited), confirmed live this checkpoint.
- **Headlines/macro:** Risk-off, yield-driven morning. The 10-year Treasury yield is at **~5.13-5.14%, a 19-year high** (last seen at this level in July 2007, three months before the GFC began — a comparison circulating widely but explicitly pushed back on by Mohamed El-Erian as "psychological anchoring" against well-telegraphed fundamentals: heavy government/tech borrowing, hawkish Fed signaling, Middle East uncertainty). The 2-year yield is at a 28-month high (~4.88-4.9%), the 1-year at a 25-month high. Futures/premarket broadly lower: S&P -0.24% to -0.65%, Nasdaq 100 -0.29% to -1.09%, Dow -0.13% to -0.48%, Russell 2000 -0.19% to -0.38% across this morning's updates. **Today's real catalysts: the Trump-Xi summit** (state dinner, tech leaders Cook/Huang/Altman attending; Bessent already announced a 2-month trade-truce extension to Jan 10) **and a heavy earnings slate** — Costco reports after the close (retail-adjacent but not a Core Ten name), Darden/TD Synnex/BlackBerry already reported this morning, all beats. Jobless claims came in lower than expected (197K, labor market resilient) — one more data point feeding the hawkish-Fed narrative. Markets pricing meaningful odds of another Fed hike after the October meeting per one read (~71%), consistent with yesterday's own PMI-driven yield surge. ([Benzinga](https://www.benzinga.com))
- **Pre-market prices, the ten proxies — C1's actual gate surface, none of this is a decision yet (RVOL/ORB can't complete before 9:45), vs yesterday's official close:** QQQ $734.79 (-0.87%) · SPY $764.29 (-0.46%) · SOXX $553.89 (**-2.09%, the sharpest move of the ten, chips extending yesterday's own weakness that produced the SOXS signal**) · IWM $281.06 (-0.31%) · XLF $54.63 (+0.16%) · XLE $63.04 (**+1.07%, the lone proxy trading higher pre-market**, crude WTI itself up over 2% on some reads) · GLD $392.76 (-0.03%, essentially flat despite the yield surge) · TLT $80.362 (-0.12%, muted given the scale of the yield move — worth watching live) · XBI $154.55 (-0.42%) · XRT $82.15 (no fresh pre-market print yet as of this checkpoint — quote timestamp matches yesterday's official close exactly, flagged stale rather than treated as a real quote). A broadly risk-off morning with energy the one clear outlier to the upside and semiconductors the clear outlier to the downside.
- **Pre-market context on the twenty leveraged tickers (not gated, informational only, C4):** TQQQ $76.58/SQQQ $35.04 (-2.64%/+2.64%) · UPRO $148.20/SPXU $34.49 (-1.40%/+1.44%) · SOXL $137.32/SOXS $35.72 (**-6.11%/+6.12%, by far the sharpest move of the field**, tracking SOXX's own steeper pullback — the same signal that ran all three of yesterday's trades looks, if anything, more stretched this morning) · TNA $59.8755/TZA $45.8093 (-0.92%/+0.99%) · FAS $150.40/FAZ $37.08 (+0.01%/flat) · ERX $103.00/ERY $9.0169 (-0.40%/-2.10%, ERX quote stamped ~12:20am ET, stale — treat as indicative only) · UGL $48.65/GLL $23.52 (-0.10%/+0.09%) · TMF $28.30/TMV $45.46 (-0.49%/+0.62%) · LABU $250.0144/LABD $7.4694 (-1.22%/+1.62%) · RETL $7.15/SZK — no fresh premarket print (null), flagged stale.
- Earnings: DRI, BB, SNX already reported this morning, all beats, none on a Core Ten theme. COST reports after today's close — also not a Core Ten name, but worth noting for XRT-adjacent context tomorrow.
...
**Watch list for 9:45, based on pre-market only (not a qualification, C1 can't run yet):** SOXX/semis is the standout again — a steeper pre-market pullback than yesterday's own pre-open read, consistent with the broad yield-driven risk-off tone. If it holds into the 9:30-9:45 range, today's first signal may look a lot like yesterday's (SOXS candidate) — worth remembering v3.83's new 2-entry cap and v3.85's decay ceiling will bind differently than yesterday if SOXS is the name again. XLE is the other proxy worth watching, still the lone one trading higher pre-market on firmer crude. TLT's muted pre-market move despite the scale of today's yield surge is worth checking live at 9:30/9:45 rather than assumed from this read.

**9:30 observation**: A1 re-checked — loss streak still **1 of 3**, flat, no resting orders, daily trade count still **0 of 3**, no instrument at its 2-entry cap. **B6 opening-range tracker initialized on the ten proxies** — each proxy's 9:30 print is the baseline C1 step 2 will check the 9:45 breakout against, vs yesterday's close: QQQ $734.84 (-0.86%) · SPY $763.91 (-0.51%) · SOXX $554.37 (**-2.01%, still the sharpest move of the ten**, holding its pre-market weakness into the open) · IWM $281.24 (-0.24%) · XLF $54.74 (+0.37%) · XLE $62.72 (**+0.56%, still the lone proxy trading higher**) · GLD $392.065 (-0.21%) · TLT $80.375 (-0.11%, still muted relative to the scale of the yield move) · XBI $154.00 (-0.77%) · XRT $82.445 (**+0.42%, first real print of the session** — the earlier "stale" pre-market quote is superseded, and it's now trading higher despite the broad risk-off tone). The pre-market pattern held cleanly into the open — SOXX/semis still the clear laggard, XLE still the clear leader. No decision made here — first real gate run is 9:45.

**9:45 — NO ENTRY. Nothing clears both steps of C1 — a correct, ordinary no-trade outcome (C5).** RVOL (20-day average volume at this same time-of-day, fresh-computed from 5-min bars, `get_equity_historicals`): **nothing clears 1.5× directly** — the whole field prints below the bar this morning: XLF ~1.299× · IWM ~1.203× · SPY ~1.136× · TLT ~0.982× · XBI ~0.862× · QQQ ~0.769× · XLE ~0.739× · SOXX ~0.674× (**worth flagging plainly: SOXX was the sharpest price mover of the morning, -2.01% at the open, but its own RVOL is actually *below* its 20-day time-of-day norm — a real price move on unremarkable relative volume, a different shape than yesterday's SOXX signal which cleared 3.26× outright**) · XRT ~0.475× · GLD ~0.392×. **v3.79 fallback triggered** — ranked by RVOL ratio, XLF tops the field at 1.299×, carried alone into step 2 (only the top of the ranking gets tested, per C1 step 1's fallback text). **Step 2: XLF's own 9:30-9:45 opening range is [$54.340, $54.815]** (from today's three 5-min bars). Live reads at 9:47 ET: $54.345, then $54.350 seconds later — **sitting almost exactly on the range low, inside it by half a cent, not a decisive break either direction.** No breakout confirmed — XLF stays inside its own opening range, fails step 2. **No candidate this checkpoint.** No trade. Falsifiable watch for 9:50: XLF decisively below $54.340 (bearish, FAZ) or reclaiming clear of the range (no signal); also re-check SOXX's RVOL next checkpoint given its price action is still the most extreme in the field even though volume hasn't confirmed it yet.

**9:50 — NO ENTRY, still flat. C1 re-run fresh, same result.** RVOL re-computed fresh (20-day average of the same 20-minute time-of-day window, `get_equity_historicals`): still nothing clears 1.5×: XLF ~1.273× · IWM ~1.185× · SPY ~1.035× · TLT ~0.966× · XBI ~0.863× · QQQ ~0.776× · XLE ~0.702× · SOXX ~0.666× · XRT ~0.472× · GLD ~0.397×. **v3.79 fallback again carries XLF alone into step 2** (still the top of the ranking). XLF's fixed 9:30-9:45 opening range is unchanged, [$54.340, $54.815] — live now $54.415, comfortably back inside the range, further from the low than at 9:45. No breakout, no candidate, no trade. Falsifiable watch for 9:55: same as before, XLF decisively below $54.340 or a fresh proxy taking over the RVOL lead.

**9:55 — NO ENTRY, still flat.** RVOL re-computed fresh (20-day average of the same time-of-day window): still nothing clears 1.5×: XLF ~1.260× · IWM ~1.133× · TLT ~1.038× · SPY ~0.942× · XBI ~0.880× · QQQ ~0.768× · XLE ~0.725× · SOXX ~0.656× · GLD ~0.452× · XRT ~0.429×. **v3.79 fallback again carries XLF alone into step 2.** XLF's fixed opening range [$54.340, $54.815] still holds — live $54.480, now drifting toward the middle of the range on a small bounce, further from both edges than at 9:50. No breakout, no candidate, no trade. Falsifiable watch for 10:00: same as before.

**10:00 — NO ENTRY, still flat.** RVOL re-computed fresh: still nothing clears 1.5×: XLF ~1.179× · TLT ~1.096× · IWM ~1.093× · XBI ~0.992× · SPY ~0.912× · QQQ ~0.748× · XLE ~0.716× · SOXX ~0.630× · GLD ~0.594× · XRT ~0.407×. **v3.79 fallback again carries XLF alone into step 2** (still the top of the ranking, though cooling — 1.299× at 9:45, 1.179× now). XLF's fixed opening range [$54.340, $54.815] still holds — live $54.470, mid-range, no breakout. No candidate, no trade. Falsifiable watch for 10:05: same as before; also watching whether XLF's RVOL keeps cooling toward the rest of the field or reasserts.

**10:05 — NO ENTRY, still flat.** RVOL re-computed fresh: still nothing clears 1.5×: XLF ~1.261× · IWM ~1.090× · XBI ~1.084× · TLT ~1.000× · SPY ~0.896× · QQQ ~0.746× · XLE ~0.691× · SOXX ~0.621× · GLD ~0.587× · XRT ~0.583×. **v3.79 fallback again carries XLF alone into step 2.** XLF's fixed opening range [$54.340, $54.815] still holds — live $54.650, trending up toward the high but not there yet (~23¢ short). No breakout, no candidate, no trade. Falsifiable watch for 10:10: XLF clearing $54.815 decisively (bullish, FAS) as the range narrows.

**10:10 — NO ENTRY, still flat.** RVOL re-computed fresh: still nothing clears 1.5×, but **XLF is tightening — 1.372×, up from 1.261× at 10:05**: XLF ~1.372× · XBI ~1.078× · IWM ~1.059× · TLT ~0.943× · SPY ~0.908× · QQQ ~0.727× · XLE ~0.658× · SOXX ~0.609× · GLD ~0.597× · XRT ~0.560×. **v3.79 fallback again carries XLF alone into step 2.** XLF's fixed opening range [$54.340, $54.815] still holds — live $54.645, essentially flat from 10:05, still ~17¢ short of the high. No breakout, no candidate, no trade. Falsifiable watch for 10:15: XLF clearing $54.815, and whether its RVOL keeps closing on 1.5×.

**10:15 — NO ENTRY, still flat.** RVOL re-computed fresh: still nothing clears 1.5×, XLF ticked back down slightly: XLF ~1.347× · XBI ~1.068× · IWM ~1.056× · TLT ~0.943× · SPY ~0.937× · QQQ ~0.718× · XLE ~0.655× · SOXX ~0.602× · GLD ~0.578× · XRT ~0.538×. **v3.79 fallback again carries XLF alone into step 2.** XLF's fixed opening range [$54.340, $54.815] still holds — live $54.565, pulled back from 10:10's $54.645, further from the high. No breakout, no candidate, no trade. Falsifiable watch for 10:20: same as before.

**10:20 — NO ENTRY, still flat.** RVOL re-computed fresh: still nothing clears 1.5×, XLF continuing to cool: XLF ~1.303× · XBI ~1.054× · IWM ~1.049× · SPY ~0.916× · TLT ~0.912× · QQQ ~0.704× · XLE ~0.663× · GLD ~0.611× · SOXX ~0.582× · XRT ~0.524×. **v3.79 fallback again carries XLF alone into step 2.** XLF's fixed opening range [$54.340, $54.815] still holds — live $54.555, essentially flat from 10:15. No breakout, no candidate, no trade. The whole field is going quiet mid-morning — worth flagging plainly that a genuinely flat day (nothing ever clearing the gate) is a live possibility, not just a slow start. Falsifiable watch for 10:25: same as before.

**10:25 — NO ENTRY, still flat.** RVOL re-computed fresh: still nothing clears 1.5×, XLF continuing to fade from its 10:10 peak: XLF ~1.262× · IWM ~1.042× · XBI ~1.034× · SPY ~0.900× · TLT ~0.880× · QQQ ~0.693× · XLE ~0.674× · GLD ~0.600× · SOXX ~0.569× · XRT ~0.518×. **v3.79 fallback again carries XLF alone into step 2.** XLF's fixed opening range [$54.340, $54.815] still holds — live $54.510, drifting down, roughly mid-range. No breakout, no candidate, no trade. Falsifiable watch for 10:30: same as before.

**10:30 — NO ENTRY, still flat. XLF clears 1.5× RVOL directly for the first time today — but still no breakout.** RVOL re-computed fresh: **XLF ~1.558×, clearing the bar directly — no fallback needed this checkpoint.** Rest of the field, for the record: XBI ~1.023× · IWM ~1.021× · SPY ~0.898× · TLT ~0.869× · QQQ ~0.691× · XLE ~0.684× · GLD ~0.584× · SOXX ~0.554× · XRT ~0.513× — none of them checked at step 2 since XLF already cleared step 1 outright. **Step 2: XLF's fixed 9:30-9:45 opening range [$54.340, $54.815] still holds — live $54.520, roughly mid-range, not testing either edge.** A real volume signal without a matching price move — flagged plainly, not treated as a substitute for the breakout test. No candidate, no trade. Falsifiable watch for 10:35: XLF decisively breaking either side of its range now that real volume is confirmed behind it.
...
**v3.85** — Decay ceiling, direct governor instruction, 2026-09-23, same day as v3.84 below. `rvol_decay_pct ≥ 30%` now blocks the re-entry outright (A1, C12) — v3.84's informational-only framing lasted under an hour. Set below the 33.1% actually observed on the trade that prompted the metric (2026-09-23's 3rd SOXS trade), meaning this rule would have blocked that specific trade — a deliberate choice, explicitly logged as a staged hypothesis from a sample of one, not settled calibration. D4 tracks every future decay-tagged re-entry, blocked or allowed, to revisit the number with real data.

**v3.84** — D4's RVOL-decay watch item (v3.81) turned into an actual formula, direct governor instruction, 2026-09-23. Prompted directly by v3.83 landing the same day: capping any instrument to at most one re-entry per day makes "RVOL decay" a single well-defined comparison rather than a multi-checkpoint trend to eyeball. `rvol_decay_pct = (RVOL_first_entry − RVOL_reentry) ÷ RVOL_first_entry × 100`, computed once at the day's one possible re-entry, stated in the entry report and trades.csv notes. Superseded within the hour by v3.85 above — see that entry.

**v3.83** — Per-instrument entry cap, direct governor instruction, 2026-09-23. No single instrument may be entered more than twice in a day, full stop — binds C12 step 0, C12 step 1's full field re-run, and v3.80's discretionary 3rd-entry path alike. Combined with the existing 3-trade daily cap and v3.80's 3rd-entry restriction: at most 2 distinct instruments traded per day, at most 1 total re-entry across the whole session. Prompted by SOXS being entered three separate times that day (9:45, 10:05 via a full C12 field re-run, 10:30 via C12 step 0) — the first two won, the third lost -2.611% on a cooling signal and turned the day's net negative on that ticker. Closes the gap where nothing previously stopped one name from supplying more than 2 of the day's 3 entries.
