...
- **4 sectors qualified** (not 5 — thin/mixed tape, no forced 5th, same honesty standard as yesterday):
  - **Semis** — proxy SMH +0.10%. Individuals: AMDL (AMD +1.06%), TSMX (TSM +1.30%). Only 2 clear C3 today — also checked and declined: NVDA -0.58%, AVGO +0.56%, MU -0.21%, SMCX -3.58%, MUU -0.03%, QCOM/TXN/LRCX/AMAT/MRVL/ON/ARM/MPWR all below +0.75% or negative.
  - **Biotech** — proxy XBI -0.69% (doesn't gate individual stocks). Plain-stock individuals (live scanner, real movers): SMMT +9.99%, ARCT +4.83%, ZYME +4.20%.
  - **Uranium** — proxy URA +1.58%. Individuals: UUUU +0.78% (clears C3), UEC +0.30%, DNN +0.27% — same trio as yesterday, smaller magnitude today.
  - **Precious metals reversal** (inverse-long) — proxy is DUST itself (+3.04%), self-referencing since no separate unleveraged inverse benchmark exists (same convention E3 uses for crypto's no-underlying names). Vehicles: DUST, ZSL (+1.80%) — ETF-only sector, ranked alongside the others, no individual-stock leg.
