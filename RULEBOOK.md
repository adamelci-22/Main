...
**Policy version: 3.57.** Bump on every rule/threshold change; record it in the commit.
...
| **9:45–10:50** management ×8 (10-min cadence, 5-min first slot) | A · B (+ C — entries valid anywhere in this window) | Holding, or flat and open to a fresh opportunity — first check is still 5 minutes after entry (v3.56), then settles into the regular 10-min cadence from 9:50 |
| **11:00** close | A · B4 · D | Exit, report, and arm tomorrow (primary) — pins the stop to current price (v3.57 — moved up from 12:00). The 15-min-cadence 11:15–11:45 stretch and the old 12:00 slot are retired outright, not just renamed. |
...
4. **Never delete the 11:00am close checkpoint (primary arming) or the 8:00pm checkpoint (backup verification, D1).** Together they replace what used to be a single point of failure.
...
**At every management checkpoint (9:45 through 11:00, one uniform 10-min cadence throughout, v3.57), the stop ratchets off the running high itself, discounted by twice the candidate's own noise band — never off the trailing average, never a fixed stage:**
...
**Every position closes the same trading day it was opened. No overnight hold, ever.** Enforced structurally, not by a deadline check: the 11:00 checkpoint (B2) pins the stop to the live price, so the position's own stop closes it, almost immediately, rather than a separate forced sell. State the intended exit at entry.
...
# PART C — ENTRY (9:00 · 9:30 · 9:40 primarily; any 9:45–11:00 checkpoint while flat)

> **No position may be opened outside 9:40–11:00 (v3.57 — close moved up from 12:00).** Multiple round trips per day, across different candidates, are now possible (limited margin, since 2026-08-20) — a fresh entry may be taken at **any** checkpoint while flat, not only 9:40, subject to C1's late-entry clause. **A position that closes mid-day gets an accelerated re-check instead of waiting for the next grid slot — see C12.**
...
- **Entries are valid at any checkpoint from 9:40 through 11:00** (v3.57 — window shortened along with the close) — no preferred-window distinction inside that range; the whole window is short enough (v3.43) that lateness within it isn't itself a signal.
- **After 11:00, none** — the window is closed for new positions regardless of what's setting up (B2/B4).
...
**Minimum ER required to enter, scaled to how forgiving the moment should be** (early moves are naturally noisier as they establish; entries later in the — now much shorter — window are into an already-maturing move and should be held to a higher bar). **Ranges are continuous — every clock time from 9:40 to 11:00 falls in exactly one row, no gaps.** This matters beyond the regular grid: a C12 mini-cycle check can land at any minute (fill-time-anchored, not just on the quarter hour), and needs an unambiguous minimum wherever it lands:
...
(v3.57 — dropped from three rows to two: the close moving from 12:00 to 11:00 retired the entire 15-min-cadence afternoon stretch the third row existed to cover, so there's no longer a "late, mature move" bracket to hold to a 0.30 bar. The remaining two rows keep v3.56's boundaries and thresholds unchanged.)
...
**Applies whenever a position closes before 11:00, regardless of why** — stop (including the 11:00 pin's near-immediate trigger, if the position is somehow re-entered and stopped again inside the same window), reversal, any other B3 exit. The moment of exit becomes an ad hoc **"9:30-equivalent,"** rather than waiting for the next regular grid slot (the uniform 10-min cadence, v3.57).
...
**Worked example, steps 2–4 together (this is the authority C10 leg 1 points back to; v3.57 — cadence is now uniform 10-min throughout, logic unchanged):** a position exits (fill) at 10:32. The 10:40 grid check discovers it; only 8 minutes have passed (`elapsed < 10`), so an ad hoc trigger arms for 10:42 (`fill_time + 10min`). At 10:42 the full gate stack runs — C10 leg 1 there compares 10:42's `bar_close` against the candidate's price *at 10:32* (step 3), not against 10:40's close. Declined → the next check is the regular 10:50 slot, and *that* leg 1 compares against the 10:42 read's own `bar_close` (step 4) — not back against 10:32 again, and not against 10:40. From there it's fully normal: 11:00 vs 10:50 — and since 11:00 is now the close (v3.57), that's the last comparison of the day.
...
**A chance to re-check, never a mandate to re-enter — everything else already in force still binds at full strength.** C5's "no read = no trade" and C9's "never force a trade" apply to the T+10 check exactly as hard as at 9:40; C9's 9:40–11:00 entry window (v3.57) still governs — a mini-cycle triggered late enough that `fill_time + 10min` would land past 11:00 simply finds no entry available, same as any other post-11:00 moment; A1's one-position gate is untouched. This rule only shortens *when* the next attempt happens, never *whether* one is allowed.
...
`9:00 · 9:30 · 9:40 · 9:45 · 9:50 · 10:00 · 10:10 · 10:20 · 10:30 · 10:40 · 10:50 · 11:00 · 8:00`

**No extended-hours slots.** As of v3.57 the trading day ends at 11:00 structurally — the 11:00 checkpoint's stop-pin (B2) closes whatever's open almost immediately, so there is nothing left to manage into the rest of the morning, the afternoon, or the evening. 8:00pm exists purely to verify tomorrow got armed (below), not to trade.

**Close moved up to 11:00 (v3.57), from 12:00.** Direct governor instruction. Entry stays at 9:40, and the first management check stays 5 minutes after entry at 9:45 (both unchanged from v3.56) — what changes is everything after: the regular 10-minute cadence now runs 9:50 through 11:00 (eight slots after the initial 9:45 one, was nine through 11:00 before this cut it short), and **the entire 15-minute-cadence afternoon stretch (formerly 11:15 through 11:45) is retired outright**, not just renamed or shortened — there is no slower cadence anymore, the day simply ends at the last 10-minute slot. 11:00 now does the same double duty 12:00 used to: last management checkpoint and close (stop-pin) in one. ET → UTC: EDT = UTC−4; after Sun Nov 1 2026, EST = UTC−5. Skip market holidays — **verify the calendar, never assume.** On an early close, end the grid at whichever of 11:00 or the early-close time comes first. **Friday arms Monday**, not the weekend.
...
- **Friday 11:00am always reports** (v3.57 — moved up from 12:00pm), trades or not — balance, every trade, loss-streak count, what was declined and why, any rulebook change. The guaranteed heartbeat. (Moved here from 8:00pm under D1's arming restructure — 8:00 is now a silent-unless-broken backup check, even on Fridays; the real weekly data already lives at the close, not eight hours later.)
...
**Tuesday 2026-09-08, 9:00 research** (first live day under v3.56 — entry at 9:40, first management at 9:45; Monday 9/7 was Labor Day, correctly skipped when arming):

- **Funding reconciled.** `get_portfolio` confirms **$492.46 buying power/cash** — exact match to Friday's $242.46 close + the reported $250 deposit. `unsettled_funds: $632.19` on `get_accounts` remains unreconciled against anything in the trade history and is **not** used for sizing; `buying_power` is the authoritative spendable figure. **Deposited capital**: `total_value ($492.46) − all-time realized P&L ($41.62) − unrealized P&L ($0)` = **$450.84**. **Floor: $225.42** (50% of deposited, E2). A2's prior open question (a pre-9/1 stale-trigger cleanup pass) is still outstanding, not urgent, noted again rather than dropped.
- **A1 loss streak recomputed fresh from `archive/trades.csv`: 0 of 3.** Both of Friday's losses (USAR -2.13%, KLAG -0.90%) plus AXTI's win are already reflected; nothing since builds the streak. Not blocked. **Position: flat, no resting orders** (`get_equity_positions`/`get_equity_orders`, verified).
- **Headlines/macro:** Friday's CPI print and elevated Fed-hike odds (60%, up from 44% a month ago per CME FedWatch) are this week's dominant macro thread — no resolution today, just background risk. Real overnight driver for commodities: Houthi strikes on Saudi energy facilities overnight pushed **Brent +1.7% to $98.70** — geopolitical, not demand-driven, explains today's energy-complex premarket strength (see commodity check below). **QCOM's scan-topping premarket move has no fresh same-day catalyst** — its news is 1-2 sessions old (Vietnam investment ask 8/28, Horizon Ultra launch 8/31, Monday's outperformance already priced) — treated as suspect pending a live C10 read, not assumed tradeable off the scan alone. **ORCL does carry a fresh, real catalyst stack**: Thursday's Q1 FY27 earnings anticipation (implied 10.58% options move per Benzinga), continued AI-infrastructure/OpenAI RPO framing, WSB buzz, and a same-day Benzinga "why is Oracle surging" writeup — premarket +3.8-4.5% across sources, real same-day driver, not stale carryover.
- **Market-wide individual-stock scan (D2 step 4), run live on the existing saved scan** `4ceac364-d887-4afc-b3e1-7cfe991001e1`. **79 names cleared** — up sharply from 29 at Friday's open, consistent with today's broader across-the-tape strength (semis/AI-infrastructure names dominating the top of the list).
- **Today's 25-name individual-stock watchlist**, ranked by day-change magnitude, wrapper-carrying names given priority fill per D2 (KLAC/AXTI/TSM/MU all carry wrappers — KLAG, AXTC, TSMX/TSMU, MUU — kept over the weaker plain movers that would otherwise have filled the bottom slots):
  1. DVLT +11.95% · 2. QCOM +9.57% · 3. ORBS +9.55% · 4. BE +7.79% · 5. RGTI +6.64% · 6. IONQ +5.79% · 7. QBTS +5.79% · 8. ORCL +5.02% · 9. UMC +4.75% · 10. INTC +4.74% · 11. HCWC +4.70% · 12. RITR +4.29% · 13. TE +4.13% · 14. NOK +4.09% · 15. GLW +3.95% · 16. FCX +3.72% · 17. EOSE +3.60% · 18. HOOD +3.32% · 19. GCTK +3.02% · 20. LAES +2.89% · 21. NBIS +2.88% · **22. KLAC +2.51% (wrapper KLAG exists — C4 rank-1 track, priority-filled)** · **23. AXTI +2.39% (wrapper AXTC exists — C4 rank-1 track, priority-filled, AXTC's own liquidity to be re-checked live per Friday's finding)** · **24. TSM +1.75% (wrapper TSMX/TSMU exists — C4 rank-1 track, priority-filled)** · **25. MU +1.67% (wrapper MUU exists — C4 rank-1 track, priority-filled)**.
- **Commodity check (D2 step 5): five of six proxies positive premarket** vs Friday's close — **COPX leading, roughly +3.6%** (last_non_reg $93.79 vs adjusted_previous_close $90.66), **URA roughly +1.6%**, **XLE roughly +0.8%** (consistent with the overnight Brent spike above), SLV and XLB both modest positive. **GLD the lone negative** (slightly down). **COPX, URA, XLE, SLV, XLB all provisionally qualify** for today's list on this premarket read — real C1 qualification still needs the formal 9:30→9:40 test and C6's two legs to clear live, per usual; GLD is excluded outright.
- Profiling (`tools/profile.py`) and affordability marking deferred to the 9:30/9:40 live checks per D2 step 7, per usual (v3.56 — the second observation now coincides with the entry checkpoint). Full 79-name shortlist is live via `run_scan` on the same saved scan id — not re-pasted here; the ranked table above is the complete individual-stock candidate set for today, plus the five provisionally-qualified commodities.

**9:30 observation — C1 Gate-1 baseline recorded, four of five provisional commodities pass leg 1.** State check: flat, no resting orders (`get_equity_positions` empty, `get_equity_orders` confirmed-state empty, both verified).

- **COPX +3.73%** ($94.04 vs $90.66), **URA +1.72%** ($46.85 vs $46.06), **XLE +1.36%** ($64.93 vs $64.06), **SLV +0.24%** ($59.961 vs $59.82) — all four positive at 9:30, clear C1 leg 1, stay live candidates into the 9:40 entry checkpoint pending legs 2/3. **XLB flipped negative** (-0.34%, $52.26 vs $52.44) — failed leg 1, dropped from today's live candidates (the late-entry clause could still rescue it if it turns positive at a later checkpoint, but it does not carry forward as a passing leg-1 read). **GLD stays negative** (-0.68%, $403.99 vs $406.77), as expected from the 9:00 premarket read — no path to entry. No new scan run (C1's own rule) — the 25-name individual-stock watchlist from 9:00 stands unchanged into the 9:40 entry checkpoint.

**9:40 entry — ENTER IONQ, 11 shares @ $43.2899 fill.** State check: flat, no resting orders (verified before placing).

- **C1 formal test on the four surviving commodities**: **COPX +4.37%** ($94.62 vs $90.66, above 9:30's +3.73%) and **URA +1.93%** ($46.9465 vs $46.06, above 9:30's +1.72%) both cleared all three legs — positive at 9:30, positive at 9:40, not below the 9:30 reading. **XLE failed leg 3** (+1.30% vs $64.89, below 9:30's +1.36% — a small real pullback) — no entry in XLE's leveraged vehicle at 9:40, late-entry clause stays open if it later exceeds 9:30's reading. **SLV failed leg 1 outright** (-0.16%, reversed negative). COPX/URA stayed live commodity candidates into C7's ranking.
- **C10 checked across the individual-stock shortlist using real 9:30→9:40 one-minute bars (bar_close vs bar_close, not point quotes)**: most of the day's biggest day-change leaders had already rolled over below their own 9:30 print by 9:40 despite still being up on the day — QBTS, RGTI, BE, ORCL, INTC, AXTI, NBIS all failed C10 leg 1 outright (bar_close below the 9:30 bar's close). Only **IONQ, EOSE, and FCX** stayed above their own 9:30 baseline. **EOSE declined on C5** — its catalyst (the Google/MN8 clean-energy deal) is 6 days old, no fresh same-day driver, same shape as Friday's GPRO decline. **IONQ and FCX both cleared every gate**: IONQ leg 2 (bounce off session_low) confirmed real (bar_high $43.3704 clears $43.07 session_low × 1.0035 stall threshold), leg 3 giveback only 24.0% (well under the 65% ceiling); FCX held cleanly near its session high throughout, minimal giveback.
- **C5 catalysts, both real and fresh, same-day**: IONQ raised full-year 2026 revenue guidance to $450-460M (from $280-290M) following the SkyWater acquisition, unveiled its Superion 256 platform, and is hosting an Investor Day at 12:30pm ET today — all same-day, company-specific. FCX's driver is copper hitting a fresh all-time record on the LME this morning (AI-datacenter copper demand colliding with a structural supply deficit) — real and dated, but a complex-wide commodity story rather than an IONQ-style idiosyncratic one.
- **Profiled IONQ, FCX, ORCL, COPX, URA** (`tools/profile.py`, 47 daily sessions each): **IONQ stop 3.61%, target 5.41%, stall threshold 0.35%, mfe_per_stop 0.644.** FCX stop 2.81%, mfe_per_stop 0.583. ORCL stop 2.98%, mfe_per_stop 0.644 (tied with IONQ) — **but ORCL failed C10 leg 1 outright** (9:40 bar_close $167.71 below its own 9:30 bar_close $169.10, despite being up on the day vs prior close), so it never entered the ranking pool despite the tied ratio. COPX mfe_per_stop 0.436, URA 0.591 — both below IONQ. **C7: IONQ ranks highest among gate-clearing candidates** (0.644 vs FCX's 0.583, the next-best survivor) — selected.
- **C4 wrapper check, searched live**: no single-stock leveraged wrapper exists for IONQ, FCX, or ORCL (`search`, all empty results) — IONQ's only leveraged ETF exposure is **XQTM** (Corgi Quantum Computing 2x Daily ETF), a multi-holding thematic basket where IONQ is just a 16.86% weighting, not a single-name wrapper — correctly excluded per the sectors/baskets retirement (v3.53), traded **plain** (rank-2).
- **Order execution**: reviewed at $43.21 ask, marketable limit $43.60, filled **$43.2899** (price improvement, $0.3101/sh better than the limit). 11 shares (floor($492.46 buying power ÷ $43.60) = 11), total cost $476.19. **C10 re-verified live immediately before placement**: $43.19 (13:44:39 UTC), comfortably above the $42.88 9:30 baseline. **Stop placed and confirmed resting**: stop_market, 11 sh, **$41.73 (−3.61%)**, confirmed via `get_equity_orders` (state=`confirmed`). Target 5.41% (~$45.63), `mfe_per_stop` 0.644. **v3.55 entry+5 catch-up check: not needed** — 9:45 already follows in exactly 5 minutes on the regular grid (fill landed at 13:44:46 UTC, under a minute before the 9:45 slot), so no ad hoc trigger armed. **Pre-commit for 9:45**: expect `bar_close` to hold at/above the trail; a close back below IONQ's own 9:30 baseline ($42.88) would be the falsified case for the "not currently falling" read that got it in.

**Real C4 miss caught within ~2 minutes: EXIT IONQ, ENTER IONX (the leveraged wrapper) instead.** The governor spotted two real, live, liquid, tight-spread 2x-long single-stock IONQ wrappers on the Robinhood app's own search — **IONX** (Defiance Daily Target 2X Long IONQ ETF) and **IONL** (GraniteShares 2x Long IONQ ETF) — neither of which this session's `search` call had surfaced (query returned zero results despite both existing). A third product, **IONC** (Corgi IONQ 2x Daily ETF), was checked and correctly excluded: its quote was stale (timestamped to Friday's close, not today) with a ~78% bid/ask spread — the same illiquidity shape as Friday's AXTC miss.

- **Why this one got switched (unlike USAR on 9/4): the position was under 3 minutes old and essentially flat/slightly positive**, not pulled back toward its stop — switching cost nothing to correct rather than compounding a real loss. Cancelled the IONQ stop (verified `cancelled`), exited IONQ via marketable limit at $43.40, filled better at **$43.7071** (small real gain: +$4.59, +0.964% on the position, r=0.267 — logged to `archive/trades.csv` as a closed round trip, `exit_reason=wrapper_switch_c4_correction`, does not count toward the loss streak since it wasn't a loss).
- **Chose IONX over IONL** on daily volume (IONX ran roughly 500K-1.2M shares/day over the past month vs IONL's 285K-1M) — the more liquid of the two real candidates.
- **Profiled IONX on its own 25 daily sessions** (`tools/profile.py`, per B1's corrected rule — never borrow another instrument's numbers): stop 4.63%, target 6.95%, stall threshold 0.71%, **mfe_per_stop 1.027** — meaningfully better risk-adjusted than IONQ's own 0.644, directly confirming C4's preference for the leveraged vehicle would have been correct from the start.
- **Order execution**: reviewed at $28.06 ask, marketable limit $28.35, filled **$28.2399** (price improvement). 17 shares (floor($497.05 buying power ÷ $28.35) = 17), total cost $480.08. **Stop placed and confirmed resting**: stop_market, 17 sh, **$26.93 (−4.63%)**, confirmed via `get_equity_orders` (state=`confirmed`). Target 6.95% (~$30.20).
- **RULE FIX applied to E3**: a single empty `search` result is no longer treated as proof no wrapper exists, especially for a candidate posting an obviously wrapper-worthy move — a differently-phrased retry is now the expected standing practice before declaring "plain stock, no wrapper" (see E3's new paragraph).
- **Pre-commit for 9:45**: the $26.93 stop on 17 sh IONX is now the live falsifiable line — a close back through it is the exit.

**9:45 management — EXIT IONX, 17 sh @ $27.6384. Fifth occurrence of the AFRM/GUSH/NUGT/USAR ratchet-breach shape, and the fastest yet — under 3.5 minutes from fill to exit.** State check: 17 sh IONX, `shares_held_for_sells=17`, stop confirmed resting at $26.93 going in.

- B1b since-entry pull (13:48:02–13:50 UTC): `bar_high` never exceeded the $28.2399 fill (13:48 bar high $28.2349, 13:49 bar high $28.085) — `run_high` stayed at the fill. `candidate_stop = 28.2399 × (1 − 2×0.71%) = $27.8388`, rounded to **$27.84**. By the time this was computed, a fresh quote showed live price already at **$27.79 and falling** — below the candidate level before any resting order could be placed there. Same mechanical shape as every prior instance in this pattern (E6): the trail's stall-based discount, computed off a peak that was really just the fill itself, caught up to a normal early pullback inside the first few minutes.
- Cancelled the still-valid $26.93 stop (verified `cancelled`), re-checked live price (continuing to slip: $27.69 → $27.56 bid), exited immediately via marketable limit at $27.30, filled better at **$27.6384** (price improvement). Net: **-$10.23, -2.13% on the position, r=-0.460**. Logged to `archive/trades.csv`, `exit_reason=trail_breached_before_placement`. **Loss streak recomputed fresh from trades.csv: 1 of 3** — not blocked.
- **Notable**: this is the same underlying name (IONQ/IONX) the wrapper switch just moved into 8 minutes earlier for being the C4-correct leveraged vehicle — the switch itself was still the right call (IONX's own profile was genuinely better risk-adjusted, mfe_per_stop 1.027 vs IONQ's 0.644), but the wrapper's wider real volatility (stop 4.63%, stall 0.71%, both roughly double IONQ's own numbers) meant a normal small pullback in the underlying became a proportionally larger, faster move in the 2x product — exactly the kind of noise the wrapper trade was correctly sized for, just not fast enough for this checkpoint cadence to catch before some giveback. Worth weighing next time a fresh, near-flat position is switched into a leveraged wrapper: the switch is still correct per C4/C7, but the *first* checkpoint after any switch deserves the same scrutiny as a fresh entry, not an assumption of safety from the "no cost to switch" read moments earlier.
- **C12 mini-cycle armed**: fill timestamp 13:51:36 UTC. Ad hoc T+10 trigger armed for ~14:03 UTC (`trig_016m4muNzuWFM2ALLP4GSJNi`). **T+0 commodity re-check**: COPX +4.77% ($94.99 vs $90.66, up from 9:40's +4.37%), URA +3.41% ($47.63 vs $46.06, up from 9:40's +1.93%) — both continuing to strengthen, stay live candidates for the T+10 gate stack.
- **Pre-commit for the T+10 check (~10:01 ET) or 9:50, whichever comes first**: no falsifiable line on an open position (flat) — the check is whether any shortlist name (individual stock or COPX/URA) clears the full gate stack fresh.

**9:50 management — ENTER URA, 10 shares @ $47.9696 fill.** State check: flat, no resting orders (verified before placing). This is the regular grid slot, running independently of (and slightly ahead of) the C12 T+10 trigger armed at the 9:45 checkpoint — per C12, the regular grid is never replaced by an ad hoc re-entry trigger, both apply.

- **COPX and URA both continued strengthening since the 9:45 T+0 read**: COPX +4.94% ($95.14 vs $90.66), URA +3.76% ($47.79 vs $46.06) at the time this checkpoint started. **C10 checked with real minute bars since 9:30 for both commodities plus FCX** (the individual-stock runner-up declined at 9:40): FCX held a clean uptrend (+6.35% on the day, only ~7% giveback from its session high) but **URA was at its own session high with zero giveback** — the cleanest read of the three, still making fresh highs as the checkpoint ran.
- **C7 ranking**: URA `mfe_per_stop` 0.591 vs FCX's 0.583 — close, but URA edges it, and URA's C10 read was materially cleaner (at session high vs FCX's real but small pullback). **C11 (ER) computed directly off the 24 one-minute closes since 9:30**: URA ER 0.458, far above the 0.15 floor — a clean trend, not chop.
- **C4 wrapper check, searched live with a second differently-phrased retry this time** (applying today's own new E3 rule): found **URAA** (Direxion Daily Uranium Industry Bull 2X ETF) — real and tradeable, but **declined on liquidity grounds**, same standard as Friday's AXTC and today's IONC: daily volume 28K-165K shares vs plain URA's own 2-6M, live spread ~0.74% (bid $28.42/ask $28.63) with the last trade print running stale relative to the quote. Given today's IONX lesson (a real wrapper's *own* higher volatility can outrun the checkpoint cadence even when liquid), a *thin* wrapper carries that same risk with less liquidity to exit into — traded **plain URA** instead (C4 rank-2c). **A second copper-miner-leveraged search (two more differently-phrased retries) came back empty both times** — reasonably confident no leveraged COPX-adjacent product exists, unlike URA's case where the second retry succeeded; the point of the new E3 rule is exactly this kind of differentiated outcome, not a blanket "always assume a wrapper exists."
- **C1/C6 recap**: URA already cleared C1's full 3-leg test at 9:40 (no late-entry clause needed). C6 legs: URA is itself the confirming miner-basket complex; today's price sits far above the prior session's entire range (not even a pullback), clears leg 2 trivially.
- **Order execution**: reviewed at $47.96 ask, marketable limit $48.30, filled **$47.9696** (price improvement). 10 shares (floor($486.82 buying power ÷ $48.30) = 10), total cost $479.70. **C10 re-verified live immediately before placement**: $47.95 (13:55:45 UTC), a fresh session high. **Stop placed and confirmed resting**: stop_market, 10 sh, **$46.77 (−2.50%)**, confirmed via `get_equity_orders` (state=`confirmed`). Target 3.75% (~$49.78), `mfe_per_stop` 0.591. **v3.55 entry+5 catch-up: not needed** — 10:00 already follows in about 4 minutes, under the 5-minute threshold. **Note**: the C12 T+10 trigger from the IONX exit (`trig_016m4muNzuWFM2ALLP4GSJNi`, ~14:03 UTC) is still armed but now moot — A1's one-position gate means it will find a position already open and skip, not a bug to chase.
- **Pre-commit for 10:00**: expect `bar_close` to hold at/above the trail; a close back below URA's own 9:30 baseline ($46.85) would be the falsified case.

**Governor note, between checkpoints: standing instruction to search harder for leveraged plays before any entry, logged to E3.** Confirmed URAA was genuinely found (not missed) at 9:50 and declined on a real liquidity look, not a rushed one — governor's instruction is to keep that discipline (exhaust search variants, give any found wrapper a real liquidity check) going forward, and explicitly not to rotate out of an already-open, performing position to chase a later-verified find. No trade action — still holding URA per instruction.

**10:00 management — HOLD URA, stop ratcheted up.** State check: 10 sh URA, `shares_held_for_sells=10`, stop confirmed resting at $46.77 going in. B1b since-entry pull (13:55:54–14:00 UTC): `bar_high` reached **$48.04** (13:56 bar) — a real new high above the $47.9696 fill.

- `run_high = max($47.9696 fill, $48.04 bar_high) = $48.04`. `candidate_stop = 48.04 × (1 − 2×0.22%) = 48.04 × 0.9956 = $47.8287`, rounded to **$47.83**. `new_stop = max($46.77, $47.83) = $47.83` — move of 2.27%, well above `min_stop_move_pct` (0.31%). **Live price re-verified before placing** ($47.975/$47.98, 13:00:19 UTC — comfortably clear of the candidate, learning applied from the IONX breach two checkpoints ago). Cancelled the $46.77 stop (verified `cancelled`), placed and verified resting: stop_market, 10 sh, **$47.83**, confirmed via `get_equity_orders` (state=`confirmed`).
...
**10:20 management — HOLD URA, stop unchanged.** State check: 10 sh URA, `shares_held_for_sells=10`, stop confirmed resting at $47.83 going in. B1b since-10:10 pull (14:12–14:20 UTC): no new high — `bar_high` topped out at $48.14 (14:12/14:16 bars, unchanged from the prior checkpoint), price consolidating $47.90–$48.14 since.
...
**C12 T+10 gate stack — ENTER BE, 1 share @ $276.2999.** Fired ~14:53 UTC per the armed trigger. State check: flat, no resting orders (verified before placing). Baseline for this cycle is each candidate's price at the URA exit's fill timestamp (14:35:52 UTC), per C12 step 3.

- **COPX, URA, and FCX — the three most-vetted names of the day — all failed C11 outright.** ER computed since the 14:35 baseline: FCX 0.066, COPX 0.089, URA 0.100 — all well below the 0.15 floor, genuinely choppy in this window despite being up on the day. Declined rather than forced, per C9/C5.
- **Checked the broader shortlist's biggest day-change movers for a real trend**: AXTI (ER 0.147, just under the floor, close but declined), EOSE (ER 0.051, chop, plus the same stale 9/2 catalyst that got it declined at 9:40), TE (ER 0.290, clears C11, but its most recent news is from 8/27 — over a week old, no fresh same-day driver, declined on C5). **BE cleared everything**: ER 0.376 (clean trend), and a still-live catalyst — Friday's S&P 500 inclusion (effective 9/21) continues to visibly drive today's price action (a Benzinga piece published this morning directly attributes today's +9% move to that same news) — dated but demonstrably still the active driver, not stale carryover like TE/EOSE.
- **Profiled BE** (`tools/profile.py`, 47 daily sessions): stop 4.73%, target 7.09%, stall threshold 0.47%, **mfe_per_stop 0.657**. **C4 wrapper check, searched live**: no single-stock leveraged wrapper exists for BE — traded plain (rank-2).
- **Order execution**: reviewed at $276.17 ask, marketable limit $278.50, filled **$276.2999** (price improvement). **1 share** (floor($485.32 buying power ÷ $278.50) = 1 — real capital constraint at this price, named per C7). **C10 re-verified live immediately before placement**: $276.03 (14:54:56 UTC), +1.18% above the $272.7867 fill-timestamp baseline. **Stop placed and confirmed resting**: stop_market, 1 sh, **$263.23 (−4.73%)**, confirmed via `get_equity_orders` (state=`confirmed`). Target 7.09% (~$295.85). **v3.55 entry+5 catch-up: not needed** — 11:00 already follows in under 5 minutes.
- **Pre-commit for 11:00**: expect `bar_close` to hold at/above the trail; a close back below BE's own fill-timestamp baseline ($272.7867) would be the falsified case.

**11:00 management — HOLD BE, stop ratcheted up. B5 hourly headlines check also run.** State check: 1 sh BE, `shares_held_for_sells=1`, stop confirmed resting at $263.23 going in. B1b since-entry pull (14:55:05–14:59 UTC): `bar_high` reached **$277.44** (14:59 bar) — a real new high above the $276.2999 fill.

- `run_high = max($276.2999, $277.44) = $277.44`. `candidate_stop = 277.44 × (1 − 2×0.47%) = 277.44 × 0.9906 = $274.8319`, rounded to **$274.83**. `new_stop = max($263.23, $274.83) = $274.83` — move of 4.41%, well above `min_stop_move_pct` (0.79%). **Live price re-verified before placing** ($276.67, 15:00:50 UTC, comfortably clear). Cancelled the $263.23 stop (verified `cancelled`), placed and verified resting: stop_market, 1 sh, **$274.83**, confirmed via `get_equity_orders` (state=`confirmed`).
- **B5 headlines check** (first check since 9:00, now past the hourly mark): a fresh Benzinga piece (10:45 ET) reiterates BE's long-term performance, no new catalyst but nothing negative either — the S&P 500 inclusion story from Friday remains the operative driver, unchanged. Noted in passing: broader market is red today (Dow -450 pts per the same news sweep) while today's shortlist names are running hot — a real divergence, not actionable but worth naming since it means today's strength is idiosyncratic/thematic (AI-infrastructure, quantum-computing DoC funding, commodity strength), not broad-market beta.
...
**C12 T+10 gate stack — ENTER QBTX, 68 shares @ $7.0399 (the leveraged wrapper of QBTS).** Fired ~15:30 UTC. State check: flat, no resting orders (verified before placing). Baseline for C10 leg 1 is each candidate's price at the BE exit's fill timestamp (15:03:49 UTC).

- **Widened the check to the broader shortlist's biggest day-change movers, all against the 15:03 baseline**: ORBS cleared C10 leg 1 but failed C11 (ER 0.103, chop). **NBIS (ER 0.663), GLW (ER 0.411), QBTS (ER 0.225), and INTC (ER 0.479) all cleared both C10 and C11** — a genuinely broader trending tape than the last two checks found.
- **Profiled all four** (`tools/profile.py`, 47 daily sessions each): NBIS mfe_per_stop 0.491 (mfe_to_target 3.05, unreachable), GLW 0.506 (mfe_to_target 2.97, unreachable), INTC 0.549 (mfe_to_target 2.73, unreachable), **QBTS 0.915 (mfe_to_target 1.64, reachable)** — QBTS ranks clearly highest, and is the only one of the four with a reachable target. **C5 catalyst confirmed fresh and named**: QBTS's subsidiary signed a definitive agreement with the U.S. Department of Commerce for up to $100M in CHIPS Act funding, reported same-day via the morning Benzinga sweep.
- **C4 wrapper check — applied today's new search-discipline instruction properly**: the first query ("D-Wave Quantum 2x leveraged ETF") returned empty, but a differently-phrased retry ("QBTS 2x") found **four real products**: QBTX (Tradr 2X Long QBTS Daily ETF, real volume 1-4M/day, tight ~0.28% spread — genuinely liquid), QBTZ (2x *short*, wrong direction, not relevant), QUA (Corgi, not liquidity-checked, skipped once QBTX confirmed liquid), QBY (GraniteShares YieldBoost — an income/covered-call product, not a directional 2x long, same category as IOYY earlier). **QBTX selected** — real, liquid, C4 rank-1.
- **Profiled QBTX on its own 25 daily sessions** (`tools/profile.py`, per B1's corrected rule): stop 5.42%, target 8.13%, stall threshold 0.74%, **mfe_per_stop 0.905** — essentially matches QBTS's own ratio while adding real leverage, the outcome C4 is built to find.
...
**Wednesday 9/9, pre-market: v3.57 close-time change applied to the already-armed grid, live, before today's session started.** Direct governor instruction ("Trading Day ends at 11 eastern") arrived after Tuesday's 12:00 close had already run and armed Wednesday's chain on the old schedule. Fixed Wednesday's grid to match rather than letting it run stale one more day: deleted the now-obsolete `trig_01GFr1ydkbrKWF3grT3smYSp` (11:15am MANAGEMENT), `trig_019gxtjg5Jg8bTVQx9zP6dta` (11:30am MANAGEMENT), `trig_01MZZRVrkKEUd7KLeoiUbmSt` (11:45am MANAGEMENT), and `trig_01RLe961t4SaySWAk2LhgnPn` (12:00pm CLOSE + PRIMARY ARMING); updated `trig_01CtWoT3cDM3bhk8PEe3eTEn` (was "11:00am MANAGEMENT") to `Wed 9/9 11:00am CLOSE + PRIMARY ARMING` with a corrected prompt. Verified via a fresh `list_triggers`: 9:00 through 10:50 unchanged, 11:00 now correctly reads CLOSE + PRIMARY ARMING, no stale 11:15/11:30/11:45/12:00 entries remain, 8:00pm backup untouched. This was done before market open (checked `updated_at` timestamps ~05:54 UTC / ~1:54am ET, well ahead of 9:00 ET) — no live checkpoint ran under the wrong schedule today. See v3.57 in Current State for the full rulebook-text changes this same instruction required.

**Also recovered from a separate operational issue while making this change: this session's local git working copy of `archive/trades.csv` had desynced from the actual remote history** — 24 rows locally vs. 30 verified-correct rows on `origin/claude/agentic-portfolio-growth-08ppdw` (missing the 9/4 AXTI governor-exit row and all six of 9/8's trades). No data was actually lost — every trade this session logged today was correctly committed and pushed at the time (verified via `git log` on the remote), the local copy just never picked up the accumulated history, for reasons not fully diagnosed (possibly a container/checkout artifact from the session picking back up after the date rolled to 9/9). Restored the correct file via `git checkout origin/... -- archive/trades.csv`, merged the diverged branch history (`git merge`, clean, no conflicts), and pushed. Flagging this here rather than silently absorbing it: **if trades.csv or RULEBOOK.md ever look wrong locally, verify against the actual remote (`git log origin/... `, `git diff origin/...`) before trusting local state** — this local copy can apparently desync from what was actually committed.
